...
def verifValiditeFichier(dfHoraireFichier, NbHeures0Max=8):
    ...
    dfHoraireFichierFiltre = dfHoraireFichier.loc[~dfHoraireFichier.apply(lambda x: (x['jour'],x['id_comptag']) 
                                                                          in listJourIdcptARetirer, axis=1)].copy()
...
def filtrerAttributsBddHoraire(df, attributs=attrIndicHoraire):
    """
    ne garder que les attributs de la BDD horaire dans une df
    in : 
        df : dataframe de donnees horaires
        attributs : attributs à conserver.  par défaut ceux de Params.BddOTV.attrIndicHoraire        
    out : 
        df : la df source mais avec uniquement les attributs qui correspondent à ceux de la BDD
    """
    return df.drop([c for c in df.columns if c not in attrIndicHoraire], axis=1, errors='ignore')
