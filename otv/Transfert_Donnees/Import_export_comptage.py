...
def compteur_existant_bdd(
    table=tableCompteur, schema=schemaComptage, dep=False, type_poste=False, gest=False
):
    """
    ...
    if not dep and not type_poste and not gest:
        ...
        rqt = (
            f"select * from {schema}.{table} where dep='{dep}' and gestionnai='{gest}'"
        )
    ...
    with ct.ConnexionBdd(nomConnBddOtv) as c:
        if table == tableCompteur:
            existant = gp.GeoDataFrame.from_postgis(
                rqt, c.sqlAlchemyConn, geom_col="geom", crs="epsg:2154"
            )
        else:
...
def recupererIdUniqComptage(dfSource, derniereAnnee=False):
    ...
    if not derniereAnnee:
        ...
        dfidComptagSource = dfSource.drop_duplicates(["id_comptag", "annee"])[
            ["id_comptag", "annee"]
        ]
    else:
    ...
    dfSourceIds = (
        dfSource.merge(
            dfIdCptUniqs, on=["id_comptag", "annee"], how="left"
        ).drop_duplicates()
        if not derniereAnnee
        else dfSource.merge(dfIdCptUniqs, on="id_comptag", how="left").drop_duplicates()
    )
    # verif que tout le monde a un id_comptag_uniq
    if dfSourceIds.id_comptag_uniq.isna().any():
        dfSourceIdsNanList = dfSourceIds.loc[
            dfSourceIds.id_comptag_uniq.isna()
        ].id_comptag.unique()
        warnings.warn(
            f"les id_comptag {dfSourceIdsNanList} n'ont pas d'id_comptag_uniq. Creer un comptage avant "
        )
    return dfSourceIds
...
def recupererIdUniqComptageAssoc(listIdComptag):
    ...
    """
    with ct.ConnexionBdd(nomConnBddOtv) as c:
        dfIdCptUniqsAssoc = pd.read_sql(
            f"select id id_comptag_uniq, id_cptag_ref, id_cpteur_asso from {schemaComptageAssoc}.{tableComptage} where id_cptag_ref=ANY(ARRAY{listIdComptag})",
            c.sqlAlchemyConn,
        )
...
def recupererComptageSansTrafic(listIdComptag, annee):
    ...
    """
    with ct.ConnexionBdd(nomConnBddOtv) as c:
        dfCptSansTmja = pd.read_sql(
            f"select id, id_comptag, annee from comptage.vue_comptage_sans_tmja where id_comptag=ANY(ARRAY{listIdComptag}) and annee='{annee}'",
            c.sqlAlchemyConn,
        )
...
def recupererLastAnnKnow(listIdComptag):
    ...
    with ct.ConnexionBdd() as c:
        dfLastAnKnow = pd.read_sql(
            f"select * from {schemaComptage}.{vueLastAnnKnow} where id_comptag = any(array{listIdComptag})",
            c.sqlAlchemyConn,
        )
    return dfLastAnKnow
...
def insert_bdd(schema, table, df, if_exists="append", geomType="POINT"):
    ...
    if isinstance(df, gp.GeoDataFrame):
        ...
        with ct.ConnexionBdd(nomConnBddOtv) as c:
            df.to_sql(
                table, c.sqlAlchemyConn, schema=schema, if_exists=if_exists, index=False
            )
    return
...
def insererSchemaComptage(df, typeData):
    ...
    insert_bdd(schemaComptage, nomTable, df, "append", "POINT")
    return
...
def insererSchemaComptageAssoc(df, typeData):
    ...
    O.checkParamValues(
        typeData,
        ["comptage", "compteur", "indicAgrege", "indicMensuel", "indicHoraire"],
    )
    if typeData == "comptage":
        nomTable = tableComptage
    elif typeData == "compteur":
        ...
        nomTable = tableIndicHoraire
    insert_bdd(schemaComptageAssoc, nomTable, df, "append", "POINT")
    return
