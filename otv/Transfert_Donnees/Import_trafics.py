...
from Donnees_horaires import (
    comparer2Sens,
    verifValiditeFichier,
    concatIndicateurFichierHoraire,
    SensAssymetriqueError,
    verifNbJoursValidDispo,
    tmjaDepuisHoraire,
    periodeDepuisHoraire,
    attributsHoraire,
    mensuelDepuisHoraire,
)
from Donnees_sources import FIM, MHCorbin, DataSensError, PasAssezMesureError
from Integration_nouveau_comptage import (
    corresp_nom_id_comptag,
    scinderComptagExistant,
    creer_comptage,
    structureBddOld2NewForm,
...
class Comptage:
    ...
    def filtrer_periode_ponctuels(self):
        ...
        self.df_attr = self.df_attr.loc[
            self.df_attr.apply(
                lambda x: x["debut_periode"].month not in [7, 8]
                and x["fin_periode"].month not in [7, 8],
                axis=1,
            )
        ].copy()
    ...
    def donnees_existantes(self, table_linearisation_existante, table_cpt):
        ...
        """
        with ct.ConnexionBdd(nomConnBddOtv) as c:
            lin_precedente = gp.GeoDataFrame.from_postgis(
                f"select * from {table_linearisation_existante}",
                c.sqlAlchemyConn,
                geom_col="geom",
                crs={"init": "epsg:2154"},
            )
            pt_cpt = pd.read_sql(
                f"select id_comptag,type_poste from comptage.{table_cpt}",
                c.sqlAlchemyConn,
            )
            lin_precedente = lin_precedente.merge(pt_cpt, how="left", on="id_comptag")
        return lin_precedente

    def plus_proche_voisin_comptage_a_inserer(
        self,
        df,
        schema_temp,
        nom_table_temp,
        ...
        points_a_inserer = self.localiser_comptage_a_inserer(
            df, schema_temp, nom_table_temp, table_linearisation_existante, table_pr
        )
        # recuperer les donnees existante
        lin_precedente = self.donnees_existantes(
            table_linearisation_existante, table_cpt
        )

        # ne conserver que le spoints a inserer pour lesquels il y a une geometrie
        points_a_inserer_geom = points_a_inserer.loc[
            ~points_a_inserer.geom.isna()
        ].copy()
        # recherche de la ligne la plus proche pour chaque point a inserer
        ppv = O.plus_proche_voisin(
            points_a_inserer_geom,
            lin_precedente[["id_ign", "geom"]],
            5,
            "id_comptag",
            "id_ign",
        )
        ...
        ppv_id_comptagLin = ppv.merge(
            lin_precedente[["id_ign", "id_comptag", "type_poste"]].rename(
                columns={"id_comptag": "id_comptag_lin", "type_poste": "type_poste_lin"}
            ),
            on="id_ign",
        )
        ppv_id_comptagLin = (
            ppv_id_comptagLin.loc[~ppv_id_comptagLin.id_comptag_lin.isna()]
            .copy()
            .merge(
                self.existant[["geom", "id_comptag"]].rename(
                    columns={"geom": "geom_cpt_lin", "id_comptag": "id_comptag_lin"}
                ),
                on="id_comptag_lin",
            )
        )
        ppv_id_comptagLin_p = gp.GeoDataFrame(
            ppv_id_comptagLin.rename(columns={"id_ign": "id_ign_cpt_new"}),
            geometry=ppv_id_comptagLin.geom_cpt_lin,
        )
        ...
        ppv_total = O.plus_proche_voisin(
            ppv_id_comptagLin_p,
            lin_precedente[["id_ign", "geom"]],
            5,
            "id_comptag_lin",
        ...
        ppv_final = (
            ppv_total.merge(ppv_id_comptagLin_p, on="id_comptag_lin")
            .merge(df[["id_comptag", "type_poste"]], on="id_comptag")
            .rename(columns={"id_ign": "id_ign_lin", "type_poste": "type_poste_new"})
            .drop_duplicates()
        )
        return ppv_final, points_a_inserer_geom

    def troncon_elemntaires(
        self, schema, table_graph, table_vertex, liste_lignes, id_name
    ):
        """
        a reprendre avec travail Vince
        """

    def corresp_old_new_comptag(
        self,
        schema_temp,
        nom_table_temp,
        table_linearisation_existante,
        ...
        ppv_final, points_a_inserer_geom = self.plus_proche_voisin_comptage_a_inserer(
            self.df_attr_insert,
            schema_temp,
            nom_table_temp,
            table_linearisation_existante,
        ...
        dico_corresp = self.troncon_elemntaires(
            schema, table_graph, table_vertex, ppv_final.id_ign_lin.tolist(), id_name
        )
        print("tronc elem fait")
        ppv_final["correspondance"] = ppv_final.apply(
            lambda x: pt_corresp(x["id_ign_lin"], x["id_ign_cpt_new"], dico_corresp),
            axis=1,
        )
        df_correspondance = ppv_final.loc[
            (ppv_final["correspondance"])
            & (~ppv_final["id_comptag_lin"].isin(self.df_attr.id_comptag.tolist()))
        ].copy()[["id_comptag_lin", "type_poste_lin", "id_comptag", "type_poste_new"]]
        ...
        valeurs_txt = str(
            tuple(
                [
                    tuple(
                        [
                            elem[i].replace("'", "_") if isinstance(i, str) else elem[i]
                            for i in range(len(liste_attr))
                        ]
                    )
                    for elem in zip(*[df[a].tolist() for a in liste_attr])
                ]
            )
        )[1:-1]
        return valeurs_txt
    ...
    def update_bdd(
        ...
        """
        rqt_attr = ",".join(
            f"{attr_b}=v.{attr_f}" for (attr_b, attr_f) in dico_attr_modif.items()
        )
    ...
    def insert_bdd(self, schema, table, df, if_exists="append", geomType="POINT"):
        ...
        if isinstance(df, gp.GeoDataFrame):
            if df.geometry.name != "geom":
                ...
                df.geom = df.apply(
                    lambda x: WKTElement(x["geom"].wkt, srid=2154), axis=1
                )
            with ct.ConnexionBdd(nomConnBddOtv) as c:
                df.to_sql(
                    table,
...
class Comptage_cd64(Comptage):
    ...
    def miseEnForme(self):
        ...
        dfBrute = dfBrute.rename(
            columns={
                c: unidecode(c).lower().replace(" ", "_").replace("%", "pc_")
                for c in dfBrute.columns
            }
        )
        dfBrute["departement"] = dfBrute["departement"].astype("str")
        dfBrute["type"] = dfBrute["type"].apply(
            lambda x: "permanent" if x == "Per" else "tournant"
        )
        dfBrute["route"] = dfBrute["route"].apply(
            lambda x: O.epurationNomRoute(x.split()[1])
        )
        ...
        dfBrute["obs_supl"] = dfBrute.apply(
            lambda x: f"section : {x.section} ; indice : {x.indice} ; sens : {x.sens} ; localisation : {x.localisation}",
            axis=1,
        )
        ...
        dfBrute["id_comptag"] = dfBrute.apply(
            lambda x: f"{x.dep}-{x.route}-{x.pr}+{x['abs']}", axis=1
        )
    ...
    def classer_comptage_insert_update(self, table="compteur", schema="comptage"):
        ...
        existant = compteur_existant_bdd(
            table, schema=schema, dep="64", type_poste=False
        )
        ...
        # classement
        self.df_attr_update = self.df_attr.loc[
            self.df_attr.id_comptag.isin(existant.id_comptag.tolist())
        ].copy()
        self.df_attr_insert = self.df_attr.loc[
            ~self.df_attr.id_comptag.isin(existant.id_comptag.tolist())
        ].copy()
        return
    ...
    def verifComptageInsert(self, dicoCorresp, gdfInsertCorrigee):
        ...
        self.insert_bdd(
            ...
            pd.DataFrame.from_dict(
                {
                    "id_gest": [k for k in dicoCorresp.keys()],
                    "id_gti": [v for v in dicoCorresp.values()],
                }
            ),
        )
        # recalcul des correspondances
        self.classer_comptage_insert_update()
        self.df_attr_insert = gp.GeoDataFrame(
            self.df_attr_insert.merge(
                gdfInsertCorrigee[["id_comptag", "geometry"]], on="id_comptag"
            )
        )
        return
...
class Comptage_cd23(Comptage):
    ...
    def ouvrirMiseEnForme(self):
        ...
        # renomer les champs
        df_excel_rennome = df_excel.rename(
            columns={
                "1er trimestre  du 01 janvier au 31 mars": "trim1_TV",
                "Unnamed: 9": "trim1_pcpl",
                "2ème trimestre du 01 avril au 30 juin": "trim2_TV",
                "Unnamed: 11": "trim2_pcpl",
                "3ème trimestre du 01 juillet au 30 septembre": "trim3_TV",
                "Unnamed: 13": "trim3_pcpl",
                "4ème trimestre du 01 octobre au 31 décembre": "trim4_TV",
                "Unnamed: 15": "trim4_pcpl",
                "Unnamed: 17": "pc_pl",
                f"TMJA {self.annee}": "tmja",
            }
        )
        # supprimer la 1ere ligne
        df_excel_filtre = df_excel_rennome.loc[1:, :].copy()
        # mise en forme attribut
        df_excel_filtre["Route"] = df_excel_filtre.apply(
            lambda x: str(x["Route"]).upper(), axis=1
        )
        # attribut id_comptag
        for i in ["DEP", "PR", "ABS"]:
            df_excel_filtre[i] = df_excel_filtre.apply(lambda x: str(int(x[i])), axis=1)
        df_excel_filtre["id_comptag"] = df_excel_filtre.apply(
            lambda x: "-".join(
                [x["DEP"], "D" + str(x["Route"]), x["PR"] + "+" + x["ABS"]]
            ),
            axis=1,
        )
    ...
    def classer_compteur_update_insert(self, table_cpt="compteur"):
        ...
        self.df_attr_update = self.df_attr.loc[
            self.df_attr.id_comptag.isin(self.existant.id_comptag.tolist())
        ].copy()
        self.df_attr_insert = self.df_attr.loc[
            ~self.df_attr.id_comptag.isin(self.existant.id_comptag.tolist())
        ].copy()
    ...
    def update_bdd_23(self, schema, table):
        valeurs_txt = self.creer_valeur_txt_update(
            self.df_attr_update, ["id_comptag", "tmja", "pc_pl", "src", "fichier"]
        )
        dico_attr_modif = {
            f"tmja_{self.annee}": "tmja",
            f"pc_pl_{self.annee}": "pc_pl",
            f"src_{self.annee}": "src",
            "fichier": "fichier",
        }
...
class Comptage_cd17(Comptage):
    ...
    def __init__(self, fichier, type_fichier, annee, delimiter=";", skiprows=0):
        ...
        self.liste_type_fichier = [
            "brochure_pdf",
            "permanent_csv_format3sens",
            "tournant_xls_bochure",
            "ponctuel_xls_bochure",
            "permanent_csv_format3sens",
            "permanent_csv_formatTmjPl",
            "ponctuel_csv",
        ]
    ...
    def lire_borchure_pdf(self):
        ...
        with open(self.fichier, encoding="utf-8") as fichier:  # ouvrir l fichier
            ...
            liste_decomposee_ligne = re.split(
                r"(Janvier|Février|Mars|Avril|Mai|Juin|Juillet|Août|Sept.|Oct.|Nov.|Déc.|Jan.)",
                "".join(liste),
            )  # permet de conserver le mois
            liste_decomposee_ligne = [
                liste_decomposee_ligne[i] + liste_decomposee_ligne[i + 1]
                for i in range(0, len(liste_decomposee_ligne) - 1, 2)
            ]  # necessaier pour repasser le mois dans le string
            liste_decomposee_ligne[0] = (
                " " + liste_decomposee_ligne[0]
    ...
    def brochure_pdf_voie_pr_abs(self):
        ...
        """
        return (
            [element.split(" ")[1] for element in self.fichier_src],
            [element.split(" ")[2] for element in self.fichier_src],
            [element.split(" ")[3] for element in self.fichier_src],
        )
    ...
    def brochure_pdf_tmj_pcpl_v85(self):
        ...
        for element in self.fichier_src:
            ...
            nombre_a_virgule = re.search(
                "[0-9]{1,}\,[0-9]{1,}", element
            )  # rechreche un truc avec deux chiffres séparés par une virgule : renvoi un objet match si ok, none sinon
            if nombre_a_virgule:
                try:
                    v85.append(
                        float(
                            element_decompose[
                                element_decompose.index(nombre_a_virgule.group()) + 1
                            ].replace(",", ".")
                        )
                    )  # idem tmj
                    ...
                    tmj.append(
                        int(
                            element_decompose[
                                element_decompose.index(nombre_a_virgule.group()) - 2
                            ]
                        )
                    )  # donc on en deduit le tmja selon al position (car seare par un ' ' dans le fichier de base)
                except ValueError:
                    pc_pl.append(
                        float(
                            element_decompose[
                    ...
                    tmj.append(
                        int(
                            element_decompose[
                                element_decompose.index(nombre_a_virgule.group()) - 3
                            ]
                        )
                    )
    ...
    def brochure_pdf_mois_periode(self):
        ...
        periode = [
            element.split()[-3] + "-" + element.split()[-2]
            for element in self.fichier_src
        ]
        return mois, periode
        ...
        return pd.DataFrame(
            {
                ...
                "pr": pr,
                "abs": absc,
                "tmja_" + str(self.annee): tmj,
                "pc_pl_" + str(self.annee): pcpl,
                "v85": v85,
                "mois": mois,
                "periode": periode,
            }
    ...
    def permanent_csv_attr(self):
        ...
        if self.type_fichier == "permanent_csv_format3sens":
            fichier_src_2sens = self.fichier_src.loc[
                self.fichier_src["Sens"] == "3"
            ].copy()
            liste_attr = (
                [a for a in fichier_src_2sens.columns if a[:6] == "MJM TV"]
                + ["Route", "PRC", "ABC"]
                + [
                    "MJA TV TCJ " + str(self.annee),
                    "MJA %PL TCJ " + str(self.annee),
                    "MJAV85 TV TCJ " + str(self.annee),
                ]
            )
            liste_nom = (
                [
            ...
            fichier_filtre = (
                fichier_src_2sens[liste_attr].rename(columns=dico_corres_mois).copy()
            )
            fichier_filtre = fichier_filtre.loc[
                ~fichier_filtre["tmja_" + str(self.annee)].isna()
            ].copy()
            fichier_filtre["tmja_" + str(self.annee)] = fichier_filtre[
                "tmja_" + str(self.annee)
            ].apply(lambda x: int(x))
            fichier_filtre["pc_pl_" + str(self.annee)] = fichier_filtre[
                "pc_pl_" + str(self.annee)
            ].apply(lambda x: float(x.strip().replace(",", ".")))
            fichier_filtre["route"] = fichier_filtre.route.apply(
                lambda x: x.split(" ")[1]
            )
            ...
            fichier_filtre["route"] = fichier_filtre.route.apply(
                lambda x: O.epurationNomRoute(x)
            )
            ...
            fichier_filtre["id_comptag"] = fichier_filtre.apply(
                lambda x: f"17-{x.route}-{x.pr}+{x['abs']}", axis=1
            )
            fichier_filtre["geom"] = fichier_filtre.apply(
                lambda x: O.reprojeter_shapely(
                    Point(x.longitude, x.latitude), "4326", "2154"
                )[1],
                axis=1,
            )
            fichier_filtre["src"] = self.type_fichier
            ...
            for c in [c for c in fichier_filtre.columns if "tmja" in c]:
                fichier_filtre[c] = fichier_filtre[c].apply(
                    lambda x: int(x) if not isinstance(x, float) else None
                )
    ...
    def permanent_csv_attrMens(self, fichier_filtre):
        ...
        """
        dfMensPerm = fichier_filtre.melt(
            id_vars="id_comptag",
            value_vars=[
                c for c in fichier_filtre.columns if "_tmja" in c or "_pc_pl" in c
            ],
            var_name="indic",
            value_name="valeur",
        )
        dfMensPerm["mois"] = dfMensPerm.indic.apply(lambda x: x.split("_")[0])
        dfMensPerm["indicateur"] = dfMensPerm.indic.apply(
    ...
    def ponctuel_csv_attr(self):
        ...
        dfBrute = pd.read_csv(
            ...
            skiprows=11,
            delimiter=";",
            parse_dates=["Début du comptage", "Fin du comptage"],
            dayfirst=True,
            encoding="LATIN1",
        ).rename(columns={"Unnamed: 0": "nom"})
        dfBrute["pr_abs"] = dfBrute.nom.apply(
            lambda x: re.search("[0-9]{1,3}\+[|0-9]{3}", x).group(0)
            if re.search("[0-9]{1,3}\+[|0-9]{3}", x)
            else None
        )
        ...
        dfBrute.rename(
            columns={"MJA TV S3": "tmja", "%PL S3": "pc_pl", "nomRoute": "route"},
            inplace=True,
        )
        dfBrute["periode"] = dfBrute.apply(
            lambda x: f"{x['Début du comptage'].strftime('%Y/%m/%d')}-{x['Fin du comptage'].strftime('%Y/%m/%d')}"
            if not pd.isnull(x["Début du comptage"])
            and not pd.isnull(x["Fin du comptage"])
            else None,
            axis=1,
        )
        dfBrute["pc_pl"] = dfBrute.pc_pl.apply(lambda x: float(x.replace(",", ".")))
        # filtre des données et cretion de l'identifiant
        dfDonneesLocalisable = dfBrute.loc[
            dfBrute.route.apply(lambda x: True if re.search("D[0-9]+", x) else False)
        ].copy()
        dfDonneesLocalisable["id_comptag"] = dfDonneesLocalisable.apply(
            lambda x: f"17-{x.route}-{x.pr}+{x['absc']}", axis=1
        )
        dfDonneesLocalisable = dfDonneesLocalisable.loc[
            (~dfDonneesLocalisable.pr.isna())
            & (~dfDonneesLocalisable.absc.isna())
            & (dfDonneesLocalisable.tmja > 0)
        ].copy()
    ...
    def ouvrir_xls_tournant_brochure(self):
        ...
        donnees_brutes["id_comptag"] = donnees_brutes.apply(
            lambda x: "17-"
            + O.epurationNomRoute(x["route"][2:].strip())
            + "-"
            + x["pr"],
            axis=1,
        )
    ...
    def ouvrir_xls_ponctuel_brochure(self):
        ...
        donnees_brutes["obs"] = donnees_brutes.apply(
            lambda x: "nouveau_point,"
            + x["debut_periode"].strftime("%d/%m/%Y")
            + "-"
            + x["fin_periode"].strftime("%d/%m/%Y")
            + ",v85_tv "
            + str(x["v85"]),
            axis=1,
        )
    ...
    def conversion_id_comptg_existant_xls_brochure(self):
        ...
        self.df_attr["id_comptag"] = self.df_attr.apply(
            lambda x: corresp_comptg.loc[
                corresp_comptg["id_gest"] == x["id_comptag"]
            ].id_gti.values[0]
            if x["id_comptag"] in corresp_comptg.id_gest.tolist()
            else x["id_comptag"],
            axis=1,
        )
    ...
    def mises_forme_bdd_brochure_pdf(self, dep, type_poste):
        ...
        if self.type_fichier in ("brochure", "permanent_csv_format3sens"):
            if self.type_fichier == "brochure":
                ...
                df_attr["obs_" + str(self.annee)] = df_attr.apply(
                    lambda x: "nouveau point,"
                    + x["periode"]
                    + ",v85_tv "
                    + str(x["v85"]),
                    axis=1,
                ...
                df_attr["type_poste"] = type_poste
                df_attr["obs_" + str(self.annee)] = df_attr.apply(
                    lambda x: "v85_tv " + str(x["v85"]), axis=1
                )
                df_attr.drop("v85", axis=1, inplace=True)
            df_attr["id_comptag"] = df_attr.apply(
                lambda x: "17-"
                + O.epurationNomRoute(x["route"])
                + "-"
                + str(x["pr"])
                + "+"
                + str(x["abs"]),
        ...
        # ANNEE 2020
            ...
            self.fichier_src.columns = [
                "idcpt_gest",
                "indicateur",
            ] + self.fichier_src.columns[2:].tolist()
            self.fichier_src.indicateur.replace(
                {"TMJ": "tmja", "PL": "pl", "%PL": "pc_pl"}, inplace=True
            )
            ...
            existant = compteur_existant_bdd(gest="CD17", type_poste="permanent")[
                ["id_comptag", "id_cpt"]
            ]
            existantNotNa = existant.loc[~existant.id_cpt.isna()]
            df_attr = existantNotNa.merge(
                self.fichier_src, left_on="id_cpt", right_on="idcpt_gest", how="left"
            )
            df_attr = (
                df_attr.loc[df_attr.indicateur.isin(["tmja", "pc_pl"])]
                .drop(["id_cpt", "idcpt_gest"], axis=1)
                .copy()
            )
            df_attr.rename(columns={"Annee": "valeur"}, inplace=True)
            df_attr["annee"] = str(self.annee)
        df_attr_insert = df_attr.loc[
            ~df_attr["id_comptag"].isin(existant.id_comptag.to_list())
        ]
        df_attr_update = df_attr.loc[
            df_attr["id_comptag"].isin(existant.id_comptag.to_list())
        ]
        self.df_attr, self.df_attr_insert, self.df_attr_update = (
            df_attr,
            df_attr_insert,
            df_attr_update,
        )
    ...
    def update_bdd_17(self, schema, table, localisation="boulot"):
        ...
        dico_attr_modif = {
            f"tmja_{self.annee}": f"tmja_{self.annee}",
            f"pc_pl_{self.annee}": f"pc_pl_{self.annee}",
            f"src_{self.annee}": "src",
            f"obs_{self.annee}": f"obs_{self.annee}",
            "fichier": "fichier",
        }
        self.update_bdd(schema, table, valeurs_txt, dico_attr_modif)
    ...
    def insert_bdd_mens(self, schema, table):
        ...
        mens[
            "donnees_type"
        ] = "tmja"  # a travailler plus tatrd si on doit extraire le tmja à partir des noms de colonnes, en lien du coup avec permanent_csv_attr()
    ...
    class CptCd17_typeFichierError(Exception):
        ...
        def __init__(self, type_fichier):
            Exception.__init__(
                self,
                f'type de fichier "{type_fichier}" non présent dans {Comptage_cd17.liste_type_fichier} ',
            )
...
class Comptage_cd19(Comptage):
    ...
    def comptage_forme(self):
        ...
        donnees_filtrees["pc_pl"] = donnees_filtrees[f"ann_{self.annee}"].apply(
            lambda x: 0
            if (pd.isna(x) or x == "x")
            else float(x.split("\n")[1].split("%")[0].replace(",", "."))
        )
    ...
    def classer_compteur_update_insert(
        ...
        """
        # creation de l'existant
        self.existant = compteur_existant_bdd(
            schema=schema_cpt, table=table_cpt, dep="19", type_poste=False
        )
        # mise a jour des noms selon la table de corresp existnate
        self.corresp_nom_id_comptag(self.df_attr)
        # classement
        ...
        corresp_cd19 = self.corresp_old_new_comptag(
            schema_temp,
            nom_table_temp,
            table_linearisation_existante,
            schema_graph,
        ...
        self.insert_bdd(
            schema_cpt,
            "corresp_id_comptag",
            corresp_cd19[["id_comptag", "id_comptag_lin"]].rename(
                columns={"id_comptag": "id_gest", "id_comptag_lin": "id_gti"}
            ),
        )
    ...
    def update_bdd_19(self, schema, table):
        valeurs_txt = self.creer_valeur_txt_update(
            self.df_attr_update, ["id_comptag", "tmja", "pc_pl", "src"]
        )
        dico_attr_modif = {
            "tmja_2019": "tmja",
            "pc_pl_2019": "pc_pl",
...
class Comptage_cd40(Comptage):
    ...
    def __init__(self, dossier, annee, donneesType="B152"):
        ...
        self.liste_fichier = [
            ...
            for chemin, dossier, files in os.walk(dossier)
    ...
    def verif_liste_fichier(self):
        ...
        return [
            fichier
            for fichier in self.liste_fichier
            if self.type_fichier(fichier) == self.donneesType
        ]
    ...
    def extraire_donnees_annuelles(self, fichier):
        ...
        if self.donneesType == "B152":
            df = pd.read_excel(
                fichier, header=None, skiprows=2
            )  # les 1eres lignes mettent le bordel dans la definition des colonnes
            ...
            df = pd.read_excel(
                fichier, header=None, skiprows=1
            )  # les 1eres lignes mettent le bordel dans la definition des colonnes
            ...
            pc_pl = float(
                df2.loc[df2[0] == int(self.annee)][149]
                .values[0]
                .split("\n")[1]
                .replace(" ", "")
                .replace(",", ".")
    ...
    def donnees_mensuelles(self, df2, id_comptag):
        ...
        """
        if self.donneesType == "B152":
            donnees_mens = df2.loc[
                [7, 18, 19], [13, 23, 30, 37, 44, 51, 58, 65, 72, 79, 86, 93, 100, 107]
            ].dropna(axis=1, how="all")
            # renommer les colonnes
            donnees_mens.columns = [
                element.replace("é", "e").replace(".", "").lower()
                for element in list(donnees_mens.loc[7])
            ]
            # remplacer l'annee en string et ne conserver
            donnees_mens = donnees_mens.drop(7).replace(
                ["D.Moy.Jour", "% PL"], ["tmja", "pc_pl"]
            )
            ...
            donnees_mens = df2.loc[df2[0] == 2020][
        ...
        for i, fichier in enumerate(self.verif_liste_fichier()):
            ...
            if self.donneesType == "B152":
                (
                    ...
                    tmjhe,
                    df2,
                ) = self.extraire_donnees_annuelles(fichier)
                self.remplir_dico(
                    dico_annee,
                    dataList,
                    compteur,
            ...
            if i == 0:
                ...
                donnees_mens_tot = pd.concat(
                    [donnees_mens_tot, donnees_mens], sort=False, axis=0
                )
    ...
    def update_bdd_40(self, schema, table):
        valeurs_txt = self.creer_valeur_txt_update(
            self.df_attr_update, ["id_comptag", "tmja", "pc_pl"]
        )
...
class Comptage_cd47(Comptage):
    ...
    def liste_dossier(self):
        ...
        liste_dossiers = [
            ...
            if "UD" in root and self.type_cpt.upper() in root
        ]
    ...
    def dico_fichier(self, liste_dossiers):
        ...
        if self.type_cpt.upper() == "TRAFICS PERMANENTS":
            dico_fichiers = {
                dossier: {"fichier": O.ListerFichierDossier(dossier, ".xlsx")}
                ...
                dossier: {
                    "fichier": [
                        [
                            ...
                            if uniq in fichier
                            and re.search("(T[1-4]|[1-4]T)", fichier)
                            and (
                                "vitesse" not in fichier.lower()
                                and not re.search(" V (T[1-4]|[1-4]T)", fichier.upper())
                            )
                        ]
                        for uniq in set(
                            [
                                re.split("[T,V][1-4]", fichier)[0]
                                for fichier in O.ListerFichierDossier(dossier, ".xlsx")
                            ]
                        )
                    ]
                ...
                dossier: {
                    "fichier": [
                        [
                            ...
                            for fichier in O.ListerFichierDossier(dossier, ".xlsx")
                            for uniq in [
                                re.split(" T.xlsx", fichier)[0]
                                for fichier in O.ListerFichierDossier(dossier, ".xlsx")
                            ]
                            if uniq in fichier
                            and re.search(" T.xlsx", fichier)
                            and "V.xlsx" not in fichier
                            and re.search("D[ ]{0,1}[0-9]+", fichier)
                        ]
                    ]
                }
    ...
    def ouverture_fichier(self, dossier, fichier):
        ...
        colonne = (
            ["jour", "type_veh"]
            + [str(i) + "_" + str(i + 1) + "h" for i in range(24)]
            + ["total", "pc_pl"]
        )
        wbFeuilles = load_workbook(
            os.path.join(dossier, fichier), read_only=True, keep_links=False
        ).sheetnames
        try:
            feuille2Sens = [a for a in wbFeuilles[::-1] if a[:2] == "S_"][0]
            data = pd.read_excel(
                os.path.join(dossier, fichier), sheet_name=feuille2Sens, header=None
            )  # on ouvre la premiere feuille ne partany de ladroite qui a un prefixe avec S_
        except IndexError:
            try:
                feuille2Sens = [
                    a for a in wbFeuilles[::-1] if "sens 3" in a.lower().strip()
                ][0]
            except IndexError:
    ...
    def id_comptag(self, data):
        ...
        if "PR" in localisation.upper():
            ...
            route = re.search(
                "D[ ]{0,1}[0-9]+", localisation.split(" PR ")[0].split("(")[1].strip()
            )[0].replace(" ", "")
    ...
    def donnees_generales(self, data):
        ...
        for (
            ...
            data.columns
        ):  # pour les cas bizrres ou les données ne sont pas à leurs places et  / ou sans calcul du pc_pl
            try:
                tmja = int(
                    data.loc[data["jour"] == "Moyenne journalière : ", c]
                    .values[0]
                    .split(" ")[0]
                )
                try:
                    pc_pl = float(
                        data.loc[
                            data.loc[data["jour"] == "Moyenne journalière : ", c].index
                            + 2,
                            c,
                        ]
                        .values[0]
                    ...
                    pc_pl = (
                        int(
                            data.loc[
                                data.loc[
                                    data["jour"] == "Moyenne journalière : ", c
                                ].index
                                + 2,
                                c,
                            ]
        ...
        test = data.loc[
            data.apply(
                lambda x: any(
                    ["Période " in a for a in x.values if isinstance(a, str)]
                ),
                axis=1,
            )
        ]
    ...
    def donnees_horaires(self, data):
        ...
        data_horaires = data_filtree.iloc[
            : data_filtree.loc[
                data_filtree["jour"].apply(
                    lambda x: x[:7] == "Moyenne" if isinstance(x, str) else False
                )
            ].index[0]
        ].copy()
        data_horaires.jour = pd.to_datetime(data_horaires.jour)
        data_horaires = data_horaires[data_horaires.columns[:-2]].set_index("jour")
        data_horaires = data_horaires.loc[
            data_horaires.type_veh.isin(["TV", "PL"])
        ].copy()
            ...
            for k, v in dico.items():
                ...
                for i, fichier in enumerate(v["fichier"]):
                    ...
                    mois = [
                        ...
                        if unidecode(debut_periode.month_name()[:4].lower()) in v
                    ][0]
                    v["mensuel"][mois] = [tmja, pc_pl]
                    if i == 0:
                        v["horaire"] = self.donnees_horaires(data.iloc[4:]).assign(
                            fichier=fichier
                        )
                    else:
                        v["horaire"] = pd.concat(
                            [
                                v["horaire"],
                                self.donnees_horaires(data.iloc[4:]).assign(
                                    fichier=fichier
                                ),
                            ],
                            sort=False,
            ...
            for k, v in dico.items():
                for liste_fichier in v["fichier"]:
                    ...
                    if liste_fichier:
                        ...
                        for fichier in liste_fichier:
                            if "Vitesse" not in fichier:
                                ...
                                (
                                    ...
                                    debut_periode,
                                    fin_periode,
                                ) = self.donnees_generales(data)
                                periode = f'{debut_periode.strftime("%Y/%m/%d")}-{fin_periode.strftime("%Y/%m/%d")}'
                                if i == 0:
                                    horaire = self.donnees_horaires(
                                        data.iloc[4:].assign(fichier=fichier)
                                    )
                                else:
                                    horaire = pd.concat(
                                        [
                        ...
                        try:
                            dico_final[id_comptag] = {
                                "tmja": int(statistics.mean(liste_tmja)),
                                "pc_pl": round(float(statistics.mean(liste_pc_pl)), 1),
                                "type_poste": "tournant",
                                "periode": " ; ".join(listePeriode),
                                "pr": pr,
                                "absc": absc,
                                "route": route,
                                "horaire": horaire,
                                "fichier": ", ".join(listUniqFichier),
                            }

                        except Exception as e:
                            print(
                                f"erreur statistique sur {os.path.join(k,fichier)} {e}"
                            )
                    ...
                    if liste_fichier:
                        if len(liste_fichier) > 1:
                            for fichier in liste_fichier:
                                ...
                                periode = f'{debut_periode.strftime("%Y/%m/%d")}-{fin_periode.strftime("%Y/%m/%d")}'
                                horaire = self.donnees_horaires(data.iloc[4:]).assign(
                                    fichier=fichier
                                )
                                dico_final[id_comptag] = {
                                    "tmja": tmja,
    ...
    def dataframe_dico(self, dico):
        ...
        df_agrege = (
            pd.DataFrame.from_dict(
                {
                    k: {x: y for x, y in v.items() if x not in ("horaire", "mensuel")}
                    for k, v in dico.items()
                },
                orient="index",
            )
            .reset_index()
            .rename(columns={"index": "id_comptag"})
        )
            ...
            for x, y in v.items():
                if x == "horaire":
                    ...
                    if i == 0:
                        ...
                        df_horaire_tot = pd.concat(
                            [df_horaire_tot, df_horaire], sort=False, axis=0
                        )
        print(df_horaire_tot.columns)
        df_horaire_tot.rename(
            columns={
                c: f"h{c.split('_')[0]}_{c.split('_')[1].replace('h','')}"
                for c in df_horaire_tot.columns
                if re.match("[0-9]+_[0-9]+h", c)
            },
            inplace=True,
        )
            ...
            for x, y in v.items():
                if x == "mensuel":
                    ...
                    if i == 0:
                        ...
                        df_mensuel_tot = pd.concat(
                            [df_mensuel_tot, df_mensuel], sort=False, axis=0
                        )
    ...
    def update_bdd_47(self, schema, table, df):
        ...
        """
        val_txt = self.creer_valeur_txt_update(
            df, ["id_comptag", "tmja", "pc_pl", "src"]
        )
        self.update_bdd(
            schema,
            table,
            val_txt,
            {
                f"tmja_{str(self.annee)}": "tmja",
                f"pc_pl_{str(self.annee)}": "pc_pl",
                f"src_{str(self.annee)}": "src",
            },
        )
    ...
    def mise_en_forme_insert(self):
        ...
        self.df_attr_insert["obs"] = self.df_attr_insert.apply(
            lambda x: f"""nouveau_point,{x['debut_periode'].strftime("%d/%m/%Y")}-{x['fin_periode'].strftime("%d/%m/%Y")}"""
            if not (pd.isnull(x["debut_periode"]) and pd.isnull(x["fin_periode"]))
            else None,
            axis=1,
        )
        self.df_attr_insert.rename(
            columns={
                "absc": "abs",
                "tmja": "tmja_" + str(self.annee),
                "pc_pl": "pc_pl_" + str(self.annee),
                "obs": "obs_" + str(self.annee),
            },
            inplace=True,
        )
    ...
    class CptCd47_typeCptError(Exception):
        ...
        def __init__(self, type_cpt):
            Exception.__init__(
                self,
                f'type de comptage "{type_cpt}" non présent dans {Comptage_cd47.liste_type_cpt} ',
            )
...
class Comptage_cd87(Comptage):
    ...
    def classer_fichiers_par_nom(self):
        ...
        """
        self.liste_nom_simple = [
            fichier
            for fichier in self.liste_fichier
            if re.search(
                "[D][( |_)]{0,1}[0-9]{1,5}([A-O]|[Q-Z]|[a-o]|[q-z]){0,3}.*PR[( |_)]{0,1}[0-9]{1,4}",
                fichier,
            )
        ]
        self.liste_nom_foireux = [
            fichier
            for fichier in self.liste_fichier
            if re.search("^[0-9]{1,4}([A-Z]|[a-z]){0,3}[( |_)]{0,1}[0-9]{4,5}", fichier)
            and fichier not in self.liste_nom_simple
        ]
    ...
    def verif_fichiers_dico(self):
        ...
        if not len(self.liste_nom_simple) + len(self.liste_nom_foireux) == len(
            [
                e
                for a in [b["fichiers"] for a in self.dico_voie.values() for b in a]
                for e in a
            ]
        ):
            nbfichierManquants = abs(
                (len(self.liste_nom_simple) + len(self.liste_nom_foireux))
                - len(
                    [
                        e
                        for a in [
                            b["fichiers"] for a in self.dico_voie.values() for b in a
                        ]
                        for e in a
                    ]
                )
            ...
            print("OK : tous fichiers dans dico")
        ...
        for v in self.dico_voie.values():
            for e in v:
                ...
                for i, f_a_tester in enumerate(liste_fichiers):
                    if i != 0:
                        if f_a_tester in [
                            a for b in dico_corresp_fichier_dbl.values() for a in b
                        ]:
                            continue
                    for f_test in liste_fichiers:
                        if f_test != f_a_tester and filecmp.cmp(
                            os.path.join(self.dossier, f_a_tester),
                            os.path.join(self.dossier, f_test),
                            shallow=True,
                        ):
    ...
    def dico_pt_cptg(self):
        ...
        for fichier in self.liste_nom_simple:
            nom_voie = O.epurationNomRoute(
                re.sub(
                    "( |_)",
                    "",
                    re.search(
                        "[D][( |_)]{0,1}[0-9]{1,5}([A-O]|[Q-Z]|[a-o]|[q-z]){0,3}",
                        fichier,
                    ).group(0),
                )
            ).upper()
            pr = re.search(
                "(PR[( |_)]{0,1})([0-9]{1,2})([+]{0,1})([0-9]{0,3})",
                re.sub("(!|\()", "+", fichier),
            ).group(2)
            absc = re.search(
                "(PR[( |+)]{0,1})([0-9]{1,2})([+]{0,1})([0-9]{0,3})",
                re.sub("(!|\(|_|x)", "+", fichier),
            ).group(4)
                ...
                for e in self.dico_voie[nom_voie]:
                    if pr == e["pr"]:
                        if absc == e["abs"]:
                            ...
                            self.dico_voie[nom_voie].append(
                                {"pr": pr, "abs": absc, "fichiers": [fichier]}
                            )
                            break
                else:
                    self.dico_voie[nom_voie].append(
        ...
        for fichier in self.liste_nom_foireux:
            ...
            absc, pr = int(re.split("( |_)", fichier)[2][-3:]), int(
                re.split("( |_)", fichier)[2][:-3]
            )
            if nom_voie in self.dico_voie.keys():
                for e in self.dico_voie[nom_voie]:
                    if pr == e["pr"]:
    ...
    def remplir_indicateurs_dico(self):
        ...
        for v in self.dico_voie.values():
            for e in v:
                if (
                    "tmja" in e.keys()
                ):  # pour pouvoir relancer le traitement sans refaire ce qui estdeja traite
                ...
                if len(e["fichiers"]) == 1:
                    ...
                    try:
                        obj_fim = FIM(
                            os.path.join(self.dossier, e["fichiers"][0]), "Message"
                        )
                        if obj_fim.date_debut.year != int(self.annee):
                            print(
                                f"le fichier {e['fichiers'][0]} ne contient pas des données de l'année {self.annee}"
                            )
                            continue
                        else:
                            (
                    ...
                    for f in e["fichiers"]:
                        try:
                            ...
                            if obj_fim.date_debut.year != int(self.annee):
                                print(
                                    obj_fim.date_debut,
                                    f"le fichier {f} ne contient pas des données de l'année {self.annee}",
                                )
                            else:
                    ...
                    e["tmja"], e["date_debut"], e["date_fin"] = (
                        int(statistics.mean(list_tmja)),
                        np.NaN,
                        np.NaN,
                    )
                    e["pc_pl"] = (
                        round(statistics.mean([p for p in list_pc_pl if p > 0]), 2)
                        if list_pc_pl
                        else np.NaN
                    )
                    e["horaire"] = pd.concat(list_dfHoraire).drop_duplicates(
                        ["jour", "indicateur"]
                    )
    ...
    def dataframe_dico(self):
        ...
        self.df_attr["id_comptag"] = self.df_attr.apply(
            lambda x: "87-" + x["route"] + "-" + str(x["pr"]) + "+" + str(x["absc"]),
            axis=1,
        )
        dfHoraire = pd.concat(
            [
                e["horaire"].assign(id_comptag=f"87-{k}-{e['pr']}+{e['abs']}")
                for k, v in self.dico_voie.items()
                for e in v
                if "tmja" in e.keys()
            ]
        ...
        dfHoraire = dfHoraire.drop_duplicates(
            [c for c in dfHoraire.columns if c != "fichier"]
        )
        # verif que pour chaque jour et id_comptage, je n'ai pas plus de 2 type d'indicateur
        dfHoraire["nb_occ"] = dfHoraire.groupby(
            ["jour", "id_comptag"]
        ).indicateur.transform(lambda x: x.count())
        if not dfHoraire.loc[dfHoraire.nb_occ > 2].empty:
            warnings.warn(
                f" il y a des id_comptage avec plus de 2 indicateurs sur certaines journées. verifiez dfHoraire"
            )
        # verif que les identifiants de comptage entre les données horaire et agreges correspondent:
        if not len(dfHoraire.id_comptag.unique()) == len(
            self.df_attr.id_comptag.unique()
        ):
            raise ValueError(
                f"le nombre d'iuddentifiant de comptage entre les données horaire ({len(dfHoraire.id_comptag.unique())}) et agregee ({len(self.df_attr.id_comptag.unique())}) n'est pas équivalent. verifiez"
            )
        self.df_attr_horaire = dfHoraire.drop("nb_occ", axis=1, errors="ignore")
        self.df_attr_mensuel = mensuelDepuisHoraire(
            self.df_attr_horaire.assign(annee=self.annee)
        )
    ...
    def filtrer_periode_ponctuels(self):
        ...
        self.df_attr = self.df_attr.loc[
            self.df_attr.apply(
                lambda x: x["date_debut"].month not in [7, 8]
                and x["date_fin"].month not in [7, 8]
                if not (pd.isnull(x["date_debut"]) and pd.isnull(x["date_fin"]))
                else True,
                axis=1,
            )
        ].copy()
    ...
    def classer_compteur_update_insert(
        ...
        self.existant = compteur_existant_bdd(
            table_cpt, schema=schema_cpt, dep="87", type_poste=False
        )
        self.df_attr_update = self.df_attr.loc[
            self.df_attr.id_comptag.isin(self.existant.id_comptag.tolist())
        ].copy()
        ...
        df_correspondance = self.corresp_old_new_comptag(
            schema_temp,
            nom_table_temp,
            table_linearisation_existante,
            schema_graph,
        ...
        with ct.ConnexionBdd(nomConnBddOtv) as c:
            corresp_comptg = pd.read_sql(rqt_corresp_comptg, c.sqlAlchemyConn)
        df_correspondance = df_correspondance[0].loc[
            ~df_correspondance[0]["id_comptag"].isin(corresp_comptg.id_gest.tolist())
        ]
        if not df_correspondance.empty:
            self.insert_bdd(
                "comptage",
                "corresp_id_comptag",
                df_correspondance.rename(
                    columns={"id_comptag_lin": "id_gti", "id_comptag": "id_gest"}
                )[["id_gest", "id_gti"]],
            )
    ...
    def update_bdd_d87(self, table_cpt, schema_cpt):
        ...
        self.df_attr_update["obs"] = self.df_attr_update.apply(
            lambda x: x["date_debut"].strftime("%d/%m/%Y")
            + "-"
            + x["date_fin"].strftime("%d/%m/%Y")
            if not pd.isnull(x["date_debut"])
            else "",
            axis=1,
        )
        self.df_attr_update.loc[
            self.df_attr_update.pc_pl.isna(), "obs"
        ] = "pc_pl inconnu"
        ...
        valeurs_txt = self.creer_valeur_txt_update(
            self.df_attr_update,
            ["id_comptag", "tmja", "pc_pl", "obs", "src", "fichier"],
        )
        dico_attr = {
            "tmja_" + self.annee: "tmja",
            "pc_pl_" + self.annee: "pc_pl",
            "obs_" + self.annee: "obs",
            "src_" + self.annee: "src",
            "fichier": "fichier",
        }
    ...
    def insert_bdd_d87(self, table_cpt, schema_cpt, nom_table_ref, nom_table_pr):
        ...
        dbl = self.df_attr_insert.loc[
            self.df_attr_insert.duplicated("id_comptag", False)
        ].copy()
        ss_dbl = self.df_attr_insert.loc[
            ~self.df_attr_insert.index.isin(dbl.index.tolist())
        ].copy()
        dbl = dbl.dropna()
        dbl_traite = (
            dbl.loc[dbl.tmja == dbl.groupby("id_comptag").tmja.transform(max)]
            .drop_duplicates()
            .copy()
        )
        ...
        self.df_attr_insert["concession"] = "N"
        self.df_attr_insert["obs"] = self.df_attr_insert.apply(
            lambda x: f"""nouveau_point,{x['date_debut'].strftime("%d/%m/%Y")}-{x['date_fin'].strftime("%d/%m/%Y")}"""
            if not (pd.isnull(x["date_debut"]) and pd.isnull(x["date_fin"]))
            else None,
            axis=1,
        )
        self.df_attr_insert.rename(
            columns={
                "absc": "abs",
                "tmja": "tmja_" + self.annee,
                "pc_pl": "pc_pl_" + self.annee,
                "obs": "obs_" + self.annee,
            },
            inplace=True,
        )
        self.df_attr_insert.drop(
            ["date_debut", "date_fin", "route"], axis=1, inplace=True
        )
    ...
    class CptCd87_ManqueFichierDansDicoError(Exception):
        ...
        def __init__(self, nbfichierManquants):
            Exception.__init__(
                self,
                f"il manque {nbfichierManquants} fichiers présents dans liste_nom_simple ou liste_nom_foireux dans le dico_voie",
            )
...
class Comptage_cd16(Comptage):
    ...
    def cpt_perm_xls(self, skiprows, cpt_a_ignorer=()):
        ...
        donnees_filtrees = donnees_brutes[
            [
                a
                for a in donnees_brutes.columns
                if not isinstance(a, int) and "Unnamed" not in a
            ]
        ].copy()
        # traiter et mettre en forme
        tmja = donnees_filtrees.loc[
            donnees_filtrees.apply(lambda x: x["Identif. Local."][-2:] == " 3", axis=1)
        ].copy()
        tmja["pr"] = tmja["PR+Distance"].apply(
            lambda x: re.split("(\.|\+|,)", str(x))[0]
        )
        tmja["absc"] = tmja["PR+Distance"].apply(
            lambda x: int(
                re.split("(\.|\+|,)", str(x))[2]
                + "0" * (4 - len(re.split("(\.|\+|,)", str(x))[2]))
            )
            if re.search("(\.|\+|,)", str(x))
            else 0
        )
        tmja["route"] = tmja["Route"].apply(lambda x: O.epurationNomRoute(x[3:]))
        tmja["id_comptag"] = tmja.apply(
            lambda x: f"16-{x['route']}-{x['pr']}+{x['absc']}", axis=1
        )
        tmja_final = (
            tmja[["pr", "absc", "route", "id_comptag", "Année"]]
            .loc[(tmja["Année"] != " ") & (~tmja["Année"].isna())]
            .rename(columns={"Année": "tmja"})
        )
        tmja_mens = tmja[
            [c for c in tmja.columns if c in [a[2] for a in dico_mois.values()]]
            + ["id_comptag"]
        ].copy()
        tmja_mens = tmja_mens.rename(
            columns={
                a: k
                for a in tmja_mens.columns
                if a != "id_comptag"
                for k, v in dico_mois.items()
                if a == v[2]
            }
        ...
        ppl["absc"] = ppl["PR+Distance"].apply(
            lambda x: int(re.split("(\.|\+|,)", str(x))[2])
            if re.search("(\.|\+|,)", str(x))
            else 0
        )
        ppl["route"] = ppl["Route"].apply(lambda x: O.epurationNomRoute(x[3:]))
        ppl["id_comptag"] = ppl.apply(
            lambda x: f"16-{x['route']}-{x['pr']}+{x['absc']}", axis=1
        )
        ppl["Année"] = ppl["Année"].apply(
            lambda x: round(float(x), 2) if x != " " else np.NaN
        )
        ppl_mens = ppl[
            [c for c in ppl.columns if c in [a[2] for a in dico_mois.values()]]
            + ["id_comptag"]
        ].copy()
        ppl_mens = ppl_mens.rename(
            columns={
                a: k
                for a in ppl_mens.columns
                if a != "id_comptag"
                for k, v in dico_mois.items()
                if a == v[2]
            }
        ...
        ppl_mens = ppl_mens.applymap(
            lambda x: round(x, 2) if isinstance(x, float) else x
        )
        ppl_final = ppl[["pr", "absc", "route", "id_comptag", "Année"]].rename(
            columns={"Année": "pc_pl"}
        )
        df_trafic = tmja_final.merge(
            ppl_final[["pc_pl", "id_comptag"]], on="id_comptag"
        )
        ...
        donnees_mens = donnees_mens.loc[
            ~donnees_mens.id_comptag.isin(cpt_a_ignorer)
        ].copy()
    ...
    def cpt_pigma(self):
        ...
        def trouverVmas(vma):
            ...
            if vma_vl in valeursVmaAdmises:
                ...
                raise ValueError(
                    f"La vitesse VL {vma_vl} n'est pas présente dans la liste des valeurs de VMA admise dans le dictionnaire de correspondace vma_vl-vma_pl"
                )
        ...
        donnees_brutes = donnees_brutes_pt[cd16_columnsFichierPtPigma].merge(
            donnees_brutes_lgn[cd16_columnsFichierLgnPigma],
            left_on=["AXE", "PLOD", "ABSD"],
            right_on=["AXE", "PRC", "ABC"],
        )
        donnees_filtrees = (
            donnees_brutes.loc[donnees_brutes.ANNEE_COMP.astype(str) == self.annee]
            .rename(columns=cd16_dicoCorrespNomColums)
            .replace(
                {
                    "type_poste": cd16_dicoCorrespTypePoste,
                    "techno": cd16_dicoCorrespTechno,
                }
            )
        )
        donnees_filtrees["id_comptag"] = donnees_filtrees.apply(
            lambda x: f"16-{x['route']}-{x['pr']}+{x['abs']}", axis=1
        )
        donnees_filtrees["src"] = "sectionnement"
        donnees_filtrees["periode"] = donnees_filtrees.apply(
            lambda x: f"{x.PERIODE_DE.replace('-', '/')}-{x.PERIODE_FI.replace('-', '/')}"
            if not pd.isnull(x.PERIODE_DE) and not pd.isnull(x.PERIODE_FI)
            else None,
            axis=1,
        )
        donnees_filtrees["type_veh"] = "tv/pl"
        # recuperation des vitesse maximales autorisées
        donnees_filtrees[["vma_vl", "vma_pl"]] = donnees_filtrees.apply(
            lambda x: trouverVmas(x.vma), axis=1, result_type="expand"
        )
        # remplacement des valeurs à 0 par du None
        donnees_filtrees.replace(
            {
                "tmja": {0: np.nan},
                "tmje": {0: np.nan},
                "pc_pl_e": {0: np.nan},
        ...
        dfCompteurPigma = donnees_filtrees[
            [c for c in donnees_filtrees.columns if c in attBddCompteur]
        ]
        dfComptagePigma = donnees_filtrees[
            [c for c in donnees_filtrees.columns if c in attrComptage]
        ]
        dfIndicAgregePigma = donnees_filtrees[
            ["id_comptag"] + cd16_attrIndicAgregePigma
        ]
    ...
    def donnees_horaires(self, donnees_pigma):
        ...
        with os.scandir(self.dossierCptTemp) as it:
            for e in it:
                if e.is_file() and e.name.lower().endswith(".fim"):
                    ...
                    listDfHoraire.append(
                        cpt.dfHoraire2Sens.assign(
                            section_cp=cpt.section_cp,
                            qualite=cpt.qualite,
                            periode=cpt.periode,
        ...
        dfHoraireId = donnees_pigma[["id_sect", "id_comptag"]].merge(
            dfHoraire, left_on="id_sect", right_on="section_cp", how="right"
        )
        # verif si des fichiers de comptages temporaires sont inconnues dans la bdd
        if dfHoraireId.id_comptag.isna().any():
            warnings.warn(
                "au moins un comptage temporaire n'est pas présent dans le fihcier pigma. recherche dans la bdd..."
            )
            with ct.ConnexionBdd(nomConnBddOtv) as c:
                refIdComptag = pd.read_sql(
                    "select id_comptag, id_sect from comptage.compteur where gestionnai = 'CD16'",
                    c.sqlAlchemyConn,
                )
            dfIdComptagInconnus = refIdComptag.merge(
                dfHoraireId.loc[dfHoraireId.id_comptag.isna()][
                    [
                        c
                        for c in dfHoraireId.columns
                        if c not in ("id_comptag", "id_sect")
                    ]
                ],
                left_on="id_sect",
                right_on="section_cp",
            ...
            if dfIdComptagInconnus.id_comptag.empty:
                warnings.warn(
                    "au moins un comptage temporaire n'est pas présent dans la bdd. créer les compteurs avant toute insertion"
                )
            dfHoraireId = pd.concat(
                [dfHoraireId.loc[dfHoraireId.id_comptag.notna()], dfIdComptagInconnus]
            )
    ...
    def creerDataIris(self, fichier, indicateur):
        ...
        data = pd.read_csv(
            ...
            usecols=[f"H{i}" for i in range(1, 25)] + ["Jours"],
        )
        data.columns = ["jour"] + [
            f"h{int(c.split('H')[1])-1}_{c.split('H')[1]}"
    ...
    def creerDfTtJoursIris(self, dossier, indicateur):
        ...
        for files in os.listdir(dossier):
            ...
            if (
                os.path.isfile(fichier)
                and fichier.endswith(".csv")
                and "_Voie" not in fichier
            ):
    ...
    def creerDfsTtjoursTtIndicIris(self):
        ...
        dataHoraireCompelete = pd.concat(
            [
                self.creerDfTtJoursIris(self.dossierIrisTv, "TV"),
                self.creerDfTtJoursIris(self.dossierIrisPl, "PL"),
            ]
        )
        ...
        dfHoraireConcat = concatIndicateurFichierHoraire(
            dfHoraireFichierFiltre, "indicateur"
        )
        # calcul des TMJAs
        ...
        dfIdsConnus, dfIdsInconnus = scinderComptagExistant(
            indic_agrege, self.annee, dep="16"
        )
        if not dfIdsInconnus.empty:
            raise ValueError(
                f"""les compteurs {', '.join(dfIdsInconnus.id_comptag.tolist())} ne sont pas présent dans la BDD. 
                                                          ils doivent l'etre pour pouvoir inserer les donnees IRIS"""
            )
        # creation dela dataframe des comptages finale
        dfComptage = dfIdsConnus.drop_duplicates(["id_comptag", "annee"])[
            ["id_comptag", "annee"]
        ].assign(src="donnees horaire IRIS", type_veh="tv/pl")
...
class Comptage_cd86(Comptage):
    ...
    def ouvrir_cpt_perm_xls(self):
        ...
        donnees_brutes_perm["route"] = donnees_brutes_perm["AXE"].apply(
            lambda x: O.epurationNomRoute(x[3:])
        )
        donnees_brutes_perm["pr"] = donnees_brutes_perm.apply(
            lambda x: int(x["CUMULD"] // 1000)
            if not pd.isnull(x["CUMULD"])
            else x["PLOD"],
            axis=1,
        )
        donnees_brutes_perm["absc"] = donnees_brutes_perm.apply(
            lambda x: int(x["CUMULD"] % 1000) if not pd.isnull(x["CUMULD"]) else 0,
            axis=1,
        )
        donnees_brutes_perm["id_gest"] = donnees_brutes_perm.apply(
            lambda x: f"86-{x['route']}-{x['pr']}+{x['absc']}", axis=1
        )
    ...
    def corresp_perm(self, table):
        ...
        corresp = (
            self.existant[["id_comptag", "route", "pr", "abs"]]
            .merge(
                self.ouvrir_cpt_perm_xls(),
                left_on=["route", "pr"],
                right_on=["route", "PLOD"],
                how="right",
            )
            .sort_values("id_comptag")
        )
        corresp["id_comptag"] = corresp.apply(
            lambda x: x["id_comptag"]
            if not pd.isnull(x["id_comptag"])
            else f"86-{x['route']}-{x['pr_y']}+{x['absc']}",
            axis=1,
        )
        # le dico de correspondance à inserer dans corresp_id_comptage
        corresp_id_comptag = (
            corresp[["id_comptag", "id_gest"]]
            .loc[corresp["id_comptag"] != corresp["id_gest"]]
            .rename(columns={"id_comptag": "id_gti"})
        )
    ...
    def forme_cpt_perm_xls(self):
        ...
        df_cpt_perm = donnees_brutes_perm[
            ["id_gest", "TMJA", "POURCENTAGE_PL", "TYPE_POSTE", "route", "pr", "absc"]
        ].rename(
            columns={
                "id_gest": "id_comptag",
                "TMJA": "tmja",
                "POURCENTAGE_PL": "pc_pl",
    ...
    def cpt_second_xls(self):
        ...
        donnees_brutes_second["route"] = donnees_brutes_second.apply(
            lambda x: "D" + str(x["RD"]).strip(), axis=1
        )
        donnees_brutes_second["pr"] = donnees_brutes_second.apply(
            lambda x: re.split("(\.|\+)", str(x["PR"]))[0], axis=1
        )
        donnees_brutes_second["absc"] = donnees_brutes_second.apply(
            lambda x: int(
                re.split("(\.|\+|,)", str(x["PR"]))[2]
                + "0" * (3 - len(re.split("(\.|\+|,)", str(x["PR"]))[2]))
            )
            if re.search("(\.|\+)", str(x["PR"]))
            else 0,
            axis=1,
        )
        donnees_brutes_second["id_comptag"] = donnees_brutes_second.apply(
            lambda x: f"86-{x['route']}-{x['pr']}+{x['absc']}", axis=1
        )
        donnees_brutes_second["type_poste"] = "tournant"
        donnees_brutes_second["% PL"] = donnees_brutes_second["% PL"] * 100
        df_cpt_second = donnees_brutes_second.rename(
            columns={"TV": "tmja", "% PL": "pc_pl"}
        ).drop(["LIEUX", "RD", "PR", "n° de compteur", "PL"], axis=1)
        return df_cpt_second

    def ouvrir_fichier_unique(self):
        donnees_brutes_perm = pd.read_excel(
            self.fichier_perm, sheet_name=self.feuil_perm
        )
        donnees_brutes_perm = donnees_brutes_perm.loc[
            (donnees_brutes_perm["ANNEE_TRAFIC"] == self.annee)
            & (~donnees_brutes_perm["PLOD"].isna())
            & (~donnees_brutes_perm["ABSD"].isna())
            & (~donnees_brutes_perm["TMJA"].isna())
            & (~donnees_brutes_perm["POURCENTAGE_PL"].isna())
        ].copy()
        donnees_brutes_perm["route"] = donnees_brutes_perm["AXE"].apply(
            lambda x: O.epurationNomRoute(x[3:])
        )
        donnees_brutes_perm["pr_cumuld"] = donnees_brutes_perm.apply(
            lambda x: int(x["CUMULD"] // 1000)
            if not pd.isnull(x["CUMULD"])
            else np.NaN,
            axis=1,
        )
        donnees_brutes_perm["absc_cumuld"] = donnees_brutes_perm.apply(
            lambda x: int(x["CUMULD"] % 1000) if not pd.isnull(x["CUMULD"]) else np.NaN,
            axis=1,
        )
        donnees_brutes_perm["id_gest_cumuld"] = donnees_brutes_perm.apply(
            lambda x: f"86-{x['route']}-{x['pr_cumuld']}+{x['absc_cumuld']}", axis=1
        )
        donnees_brutes_perm["pr_plod"] = donnees_brutes_perm.apply(
            lambda x: int(x["PLOD"]) if not pd.isnull(x["PLOD"]) else np.NaN, axis=1
        )
        donnees_brutes_perm["absc_absd"] = donnees_brutes_perm.apply(
            lambda x: int(x["ABSD"]) if not pd.isnull(x["ABSD"]) else np.NaN, axis=1
        )
        donnees_brutes_perm["id_gest_plod"] = donnees_brutes_perm.apply(
            lambda x: f"86-{x['route']}-{x['pr_plod']}+{x['absc_absd']}", axis=1
        )
        donnees_brutes_perm["POURCENTAGE_PL"] = (
            donnees_brutes_perm["POURCENTAGE_PL"] * 100
        )
        donnees_brutes_second = pd.read_excel(
            self.fichier_perm,
            sheet_name="secondaires",
            names=donnees_brutes_perm.columns,
            header=None,
        )
        donnees_brutes_second = donnees_brutes_second.loc[
            (donnees_brutes_second["ANNEE_TRAFIC"] == 2019)
            & (~donnees_brutes_second["PLOD"].isna())
            & (~donnees_brutes_second["ABSD"].isna())
            & (~donnees_brutes_second["TMJA"].isna())
            & (~donnees_brutes_second["POURCENTAGE_PL"].isna())
        ].copy()
        donnees_brutes_second["route"] = donnees_brutes_second["AXE"].apply(
            lambda x: O.epurationNomRoute(x[3:])
        )
        donnees_brutes_second["pr_cumuld"] = donnees_brutes_second.apply(
            lambda x: int(x["CUMULD"] // 1000)
            if not pd.isnull(x["CUMULD"])
            else np.NaN,
            axis=1,
        )
        donnees_brutes_second["absc_cumuld"] = donnees_brutes_second.apply(
            lambda x: int(x["CUMULD"] % 1000) if not pd.isnull(x["CUMULD"]) else np.NaN,
            axis=1,
        )
        donnees_brutes_second["id_gest_cumuld"] = donnees_brutes_second.apply(
            lambda x: f"86-{x['route']}-{x['pr_cumuld']}+{x['absc_cumuld']}", axis=1
        )
        donnees_brutes_second["pr_plod"] = donnees_brutes_second.apply(
            lambda x: int(x["PLOD"]) if not pd.isnull(x["PLOD"]) else np.NaN, axis=1
        )
        donnees_brutes_second["absc_absd"] = donnees_brutes_second.apply(
            lambda x: int(x["ABSD"]) if not pd.isnull(x["ABSD"]) else np.NaN, axis=1
        )
        donnees_brutes_second["id_gest_plod"] = donnees_brutes_second.apply(
            lambda x: f"86-{x['route']}-{x['pr_plod']}+{x['absc_absd']}", axis=1
        )
        donnees_brutes_second["POURCENTAGE_PL"] = (
            donnees_brutes_second["POURCENTAGE_PL"] * 100
        )
        donnees_concat = pd.concat(
            [donnees_brutes_perm, donnees_brutes_second], axis=0, sort=False
        )
        dico_corresp_type_poste = {
            "Permanent": "permanent",
            "Tournant": "ponctuel",
            "Secondaire": "tournant",
        }
        donnees_concat.TYPE_POSTE = donnees_concat.TYPE_POSTE.apply(
            lambda x: dico_corresp_type_poste[x]
        )
    ...
    def equivalence_id_comptag(self, table_cpt, df):
        ...
        with ct.ConnexionBdd(nomConnBddOtv) as c:
            list_corresp_comptg = pd.read_sql(
                rqt_corresp_comptg, c.sqlAlchemyConn
            ).id_gest.tolist()
            list_id_comptg = gp.GeoDataFrame.from_postgis(
                rqt_id_comtg,
                c.sqlAlchemyConn,
                geom_col="geom",
                crs={"init": "epsg:2154"},
        ...
        df["id_comptag"] = df.apply(
            lambda x: maj_id_comptg(
                x["id_gest_plod"],
                x["id_gest_cumuld"],
                list_id_comptg,
                list_corresp_comptg,
    ...
    def comptage_forme(self, table_cpt):
        ...
        if not self.feuil_perm:
            self.df_attr = pd.concat(
                [self.forme_cpt_perm_xls(), self.cpt_second_xls()], axis=0, sort=False
            )
            ...
            self.df_attr = (
                donnees_concat[
                    ["TMJA", "POURCENTAGE_PL", "TYPE_POSTE", "route", "id_comptag"]
                ]
                .rename(
                    columns={
                        a: a.lower()
                        for a in donnees_concat[
                            [
                                "TMJA",
                                "POURCENTAGE_PL",
                                "TYPE_POSTE",
                ...
                .rename(columns={"pourcentage_pl": "pc_pl"})
            )
            self.df_attr["pr"] = self.df_attr.id_comptag.apply(
                lambda x: int(x.split("-")[2].split("+")[0])
            )
            self.df_attr["absc"] = self.df_attr.id_comptag.apply(
...
class Comptage_cd24(Comptage):
    ...
    def cpt_perm_csv(self):
        def creerPeriode(serie):
            listPeriode = []
            for i in range(1, 5):
                if not pd.isnull(serie[f"DDPeriode{i}"]) and not pd.isnull(
                    serie[f"DFPeriode{i}"]
                ):
                    listPeriode.append(
                        f"{pd.to_datetime(serie[f'DDPeriode{i}'], dayfirst=True).strftime('%Y/%m/%d')}-{pd.to_datetime(serie[f'DFPeriode{i}'], dayfirst=True).strftime('%Y/%m/%d')}"
                    )
            if not listPeriode:
                return None
        ...
        donnees_traitees["Data"] = donnees_traitees.Route.apply(
            lambda x: x.split(" ")[1] if not pd.isnull(x) else None
        )
        donnees_traitees = (
            donnees_traitees.loc[donnees_traitees["Sens"] == "3"]
            .rename(columns={"MJA": "tmja", "MJAPPL": "pc_pl"})
            .copy()
        )
        donnees_traitees["id_comptag"] = donnees_traitees.apply(
            lambda x: f"24-{x['Data']}-{x['PRC']}+{x['ABC']}", axis=1
        )
        donnees_traitees["Latitude"] = donnees_traitees.Latitude.apply(
            lambda x: float(x.replace(",", "."))
        )
        donnees_traitees["Longitude"] = donnees_traitees.Longitude.apply(
            lambda x: float(x.replace(",", "."))
        )
        donnees_traitees["tmja"] = donnees_traitees.tmja.apply(lambda x: int(x))
        donnees_traitees["pc_pl"] = donnees_traitees.pc_pl.apply(
            lambda x: float(x.replace(",", "."))
        )
        donnees_traitees.rename(
            columns={"Data": "route", "PRC": "pr", "ABC": "abs"}, inplace=True
        )
        gdf_finale = gp.GeoDataFrame(
            donnees_traitees,
            geometry=gp.points_from_xy(
                donnees_traitees.Longitude, donnees_traitees.Latitude
            ),
            crs="epsg:4326",
        )
        gdf_finale = gdf_finale.to_crs("epsg:2154")
        gdf_finale["type_poste"] = gdf_finale.apply(
            lambda x: "permanent" if x["Type"].lower() == "per" else "tournant", axis=1
        )
        gdf_finale["geometry"] = gdf_finale.apply(
            lambda x: 0 if x["Latitude"] == 0 else x["geometry"], axis=1
        )
        ...
        gdf_finale.loc[
            gdf_finale.type_poste == "tournant", "periode"
        ] = gdf_finale.apply(lambda x: creerPeriode(x), axis=1)
    ...
    def comptage_forme(self):
        ...
        # mensuel permanent
        donnees_mens_perm = donnees_finales.loc[
            donnees_finales.type_poste == "permanent"
        ][
            [
                a
                for a in donnees_finales.columns
                if a in [m for e in dico_mois.values() for m in e]
            ]
            + ["id_comptag"]
        ].copy()
        donnees_mens_perm.rename(
            columns={
                c: k
                for c in donnees_mens_perm.columns
                if c != "id_comptag"
                for k, v in dico_mois.items()
                if c in v
            },
        ...
        for j, e in enumerate(
            donnees_finales.loc[donnees_finales.type_poste == "tournant"].itertuples()
        ):
            for i in range(1, 5):
                if not pd.isnull(getattr(e, f"DDPeriode{i}")) and not pd.isnull(
                    getattr(e, f"DFPeriode{i}")
                ):
                    mois = [
                        k
                        for k, v in dico_mois.items()
                        ...
                        == Counter(
                            pd.date_range(
                                pd.to_datetime(
                                    getattr(e, f"DDPeriode{i}"), dayfirst=True
                                ),
                                pd.to_datetime(
                                    getattr(e, f"DFPeriode{i}"), dayfirst=True
                                ),
                            ).month
                        ).most_common()[0][0]
                    ][0]
                    ...
                    donnees_mens_tour.loc[j + 1000, mois] = float(
                        getattr(e, f"MJPPL{i}").replace(",", ".")
                    )
                    donnees_mens_tour.loc[j, "id_comptag"] = getattr(e, f"id_comptag")
                    donnees_mens_tour.loc[j + 1000, "id_comptag"] = getattr(
                        e, f"id_comptag"
                    )
                    donnees_mens_tour.loc[j + 1000, "donnees_type"] = "pc_pl"
        # global
        self.df_attr_mens = pd.concat(
            [donnees_mens_tour, donnees_mens_perm], axis=0
...
class Comptage_cd33(Comptage):
    ...
    def __init__(
        self,
        dossier,
        fichierPermanentExcel,
        fichierTournantExcel,
        fichierPermanentShape,
        fichierTournantShape,
        fichierEnqueteShape,
        annee,
        epsg="EPSG:2154",
    ):
    ...
    def ouvertureFichier(self):
        ...
        dfTournantExcel = pd.read_excel(
            self.fichierTournantExcel,
            converters={
                "ddpériode1": lambda x: pd.to_datetime(x, dayfirst=True),
    ...
    def miseEnFormeExcel(self, dfPermanentExcel, dfTournantExcel):
        ...
        out :
            ...
            dfTournExcelFiltre
        """
        dfTournantExcelColsOk = (
            dfTournantExcel.drop("annee", axis=1, errors="ign")
            .drop(
                [
                    c
                    for c in dfTournantExcel.columns
                    if c
                    not in [e.lower() for e in cd33_dicoAttrTournExcel.keys()]
                    + [v.lower() for v in cd33_dicoAttrTournExcel.values()]
                ],
                axis=1,
                errors="ignore",
            )
            .rename(columns={k.lower(): v for k, v in cd33_dicoAttrTournExcel.items()})
        )
        dfPermanentExcelColsOk = dfPermanentExcel.drop(
            [
                c
                for c in dfPermanentExcel.columns
                if c not in cd33_dicoAttrPermExcel.keys()
            ],
            axis=1,
            errors="ignore",
        ).rename(columns={k: v for k, v in cd33_dicoAttrPermExcel.items()})
        # limitation des données à l'année en cours, sur le sens désiré, sans valeur nulle dans le tmja
        dfPermExcelFiltre = dfPermanentExcelColsOk.loc[
            (dfPermanentExcelColsOk.annee == int(self.annee))
            & (dfPermanentExcelColsOk.sens == 3)
            & (dfPermanentExcelColsOk.tmja.notna())
        ].copy()
        dfTournExcelFiltre = dfTournantExcelColsOk.loc[
            (dfTournantExcelColsOk.annee == int(self.annee))
            & (dfTournantExcelColsOk.sens == 3)
            & (dfTournantExcelColsOk.tmja.notna())
        ].copy()
        ...
        dfTournExcelFiltre["periode"] = dfTournExcelFiltre.apply(
            lambda x: " ; ".join(
                [
                    "-".join([c[0].strftime("%Y/%m/%d"), c[1].strftime("%Y/%m/%d")])
                    for c in [
                        [x.debut_periode1, x.fin_periode1],
                        [x.debut_periode2, x.fin_periode2],
                        [x.debut_periode3, x.fin_periode3],
                        [x.debut_periode4, x.fin_periode4],
                    ]
    ...
    def miseEnFormeShape(self, gdfPermanentShape, gdfTournantShape, gdfEnqueteShape):
        ...
        """
        gdfPermanentColsOk = (
            gdfPermanentShape.drop(
                [
                    c
                    for c in gdfPermanentShape.columns
                    if c not in cd33_dicoAttrPermTournShape.keys()
                ],
                axis=1,
                errors="ignore",
            )
            .rename(columns=cd33_dicoAttrPermTournShape)
            .assign(type_poste="permanent", src="shape + excel")
        )
        gdfTournantColsOk = (
            gdfTournantShape.drop(
                [
                    c
                    for c in gdfTournantShape.columns
                    if c not in cd33_dicoAttrPermTournShape.keys()
                ],
                axis=1,
                errors="ignore",
            ...
            .rename(columns=cd33_dicoAttrPermTournShape)
            .assign(type_poste="tournant", src="shape + excel")
        )
        gdfEnqueteColsOk = (
            gdfEnqueteShape.drop(
                [
                    c
                    for c in gdfEnqueteShape
                    if c
                    not in list(cd33_dicoAttrPermTournShape.keys())
                    + list(cd33_dicoAttrEnqueteShape.keys())
                ],
                axis=1,
                errors="ignore",
            )
            .rename(columns=cd33_dicoAttrPermTournShape)
            .rename(columns=cd33_dicoAttrEnqueteShape)
            .assign(type_poste="ponctuel", src="enquete")
        )
        for e in (gdfPermanentColsOk, gdfTournantColsOk, gdfEnqueteColsOk):
            if e.empty:
                continue
            e["route"] = e.route.apply(
                lambda x: O.epurationNomRoute(x) if not pd.isnull(x) else None
            )
            e["id_comptag"] = e.apply(
                lambda x: f'33-{x.route}-{int(x.pr)}+{int(x["abs"])}'
                if x[["route", "pr", "abs"]].notna().all().all()
                else None,
                axis=1,
            )
            e["dep"] = "33"
            ...
            if "troncon" in e.columns:
                e["id_sect"] = e.troncon
            e["obs_supl"] = e.rattacheme.apply(
                lambda x: f"rattaché à l'id_cpt {x}" if not pd.isnull(x) else None
            )
            # print(e)
            e["identifiant_modif"] = e["id_cpt"].str[:-2]
            dfTronconValueCount = (
                e.drop_duplicates(["identifiant_modif", "sens"])
                .identifiant_modif.value_counts()
                .rename("nb_sens")
                .reset_index()
                .rename(columns={"index": "identifiant_modif"})
            )
            e.loc[
                e.identifiant_modif.isin(
                    dfTronconValueCount.loc[
                        dfTronconValueCount.nb_sens >= 3
                    ].identifiant_modif.tolist()
                ),
                "sens_cpt",
            ] = "double sens"
            e.loc[
        ...
        if not gdfPermanentColsOk.empty:
            gdfPermanentFiltre = gdfPermanentColsOk.loc[
                (gdfPermanentColsOk.sens == 3)
                & (gdfPermanentColsOk.geometry.notna())
                & (gdfPermanentColsOk.id_comptag.notna())
            ].copy()
            gdfPermanentFiltre = O.gp_changer_nom_geom(gdfPermanentFiltre, "geom")
        else:
            gdfPermanentFiltre = None
        if not gdfTournantColsOk.empty:
            gdfTournantFiltre = gdfTournantColsOk.loc[
                (gdfTournantColsOk.sens == 3)
                & (gdfTournantColsOk.geometry.notna())
                & (gdfTournantColsOk.id_comptag.notna())
            ].copy()
            gdfTournantFiltre = O.gp_changer_nom_geom(gdfTournantFiltre, "geom")
        else:
            gdfTournantFiltre = None
        if not gdfEnqueteColsOk.empty:
            gdfEnqueteFiltre = gdfEnqueteColsOk.loc[
                (gdfEnqueteColsOk.sens == 3)
                & (gdfEnqueteColsOk.geometry.notna())
                & (gdfEnqueteColsOk.annee == int(self.annee))
                & (gdfEnqueteColsOk.id_comptag.notna())
            ].copy()
            ...
            gdfEnqueteFiltre["periode"] = gdfEnqueteFiltre.apply(
                ...
                if not pd.isnull([x.debut_periode, x.fin_periode]).all()
                else None,
                axis=1,
            )
        ...
        for j, e in enumerate(dfTournExcelFiltre.itertuples()):
            for i in range(1, 5):
                ...
                mois = [
                    ...
                    for k, v in dico_mois.items()
                    if v[0]
                    == Counter(pd.date_range(deb, fin).month).most_common()[0][0]
                ][0]
                # print(e)
    ...
    def syntheseNouvellesDonnees(
        ...
        if len(gdfEnqueteFiltre) != len(
            gdfEnqueteColsOk.loc[
                (gdfEnqueteColsOk.sens == 3)
                & (gdfEnqueteColsOk.geometry.notna())
                & (gdfEnqueteColsOk.annee == int(self.annee))
            ]
        ...
        dfTtSources = gp.GeoDataFrame(
            pd.concat(
                [
                    ...
                    for e in (gdfPermTrafic, gdfTournTrafic, gdfEnqueteFiltre)
                ]
            ),
            geometry="geom",
            crs=self.epsg,
    ...
    def miseEnFormeGenerale(self):
        ...
        (
            ...
            gdfTournantShape,
            gdfEnqueteShape,
        ) = self.ouvertureFichier()
        dfPermExcelFiltre, dfTournExcelFiltre = self.miseEnFormeExcel(
            dfPermanentExcel, dfTournantExcel
        )
        (
            gdfPermanentFiltre,
            gdfTournantFiltre,
            gdfEnqueteFiltre,
            gdfEnqueteColsOk,
        ) = self.miseEnFormeShape(gdfPermanentShape, gdfTournantShape, gdfEnqueteShape)
        donnees_mens_tour = self.miseEnFormeTournant(dfTournExcelFiltre)
        dfTtSources = self.syntheseNouvellesDonnees(
            dfPermExcelFiltre,
            gdfPermanentFiltre,
            dfTournExcelFiltre,
            gdfTournantFiltre,
    ...
    def old_analysePerm(self, sheet_name="Permanents"):
        ...
        gdfPerm = gp.GeoDataFrame(
            perm,
            geometry=gp.points_from_xy(perm.Longitude, perm.Latitude),
            crs="EPSG:4326",
        )
        ...
        gdfPerm.rename(
            columns={
                c: c.replace("MJA TV TCJ ", "tmja_")
                if "TV" in c
                else c.replace("MJA %PL TCJ ", "pc_pl_")
                for c in gdfPerm.columns
                if any([a in c for a in ("MJA TV TCJ", "MJA %PL TCJ")])
            },
            inplace=True,
        )
        gdfPerm.rename(
            columns={
                c: [k for k, v in dico_mois.items() if int(c.split()[3][:2]) in v][0]
                for c in gdfPerm.columns
                if "MJM TV TCJ" in c
            },
            inplace=True,
    ...
    def old_trierPermConnus(self, table, localisation):
        ...
        self.existant = compteur_existant_bdd(
            table, schema="comptage", localisation=localisation, dep="33", gest="CD33"
        )
        ...
        GdfPerm = gdfPerm[
            ...
            + [k for k in dico_mois.keys()]
        ].merge(
            self.existant.loc[~self.existant[f"tmja_{self.annee-1}"].isna()][
                ["id_comptag", f"tmja_{self.annee-1}"]
            ],
            how="left",
        )
    ...
    def old_analyseTourn(self, sheet_name="Tournants"):
        ...
        gdfTourn[f"obs_{self.annee}"] = gdfTourn.apply(
            lambda x: " ; ".join(
                [
                    f"{x[f'DDPériode{i}'].date()}-{x[f'DFPériode{i}'].date()}"
                    for i in range(1, 5)
                    if not (
                        pd.isnull(x[f"DDPériode{i}"]) and pd.isnull(x[f"DFPériode{i}"])
                    )
                ]
            ),
            axis=1,
    ...
    def old_donneesMensTournant(self, gdfTourn):
        ...
        for j, e in enumerate(gdfTourn.itertuples()):
            for i in range(1, 5):
                try:
                    mois = [
                        ...
                        == Counter(
                            pd.date_range(
                                pd.to_datetime(
                                    getattr(e, f"DDPériode{i}"), dayfirst=True
                                ),
                                pd.to_datetime(
                                    getattr(e, f"DDPériode{i}"), dayfirst=True
                                ),
                ...
                donnees_mens_tour.loc[j, "donnees_type"] = "tmja"
                donnees_mens_tour.loc[j + 1000, mois] = round(
                    getattr(e, f"MJPPL{i}") / getattr(e, f"MJPTV{i}") * 100, 2
                )
                donnees_mens_tour.loc[j, "troncon"] = getattr(e, f"troncon")
                donnees_mens_tour.loc[j + 1000, "troncon"] = getattr(e, f"troncon")
                donnees_mens_tour.loc[j + 1000, "donnees_type"] = "pc_pl"
    ...
    def old_correspondanceTournant(
        self,
        localisation,
        rqtPpvCptTournant,
        rqtCptExistant,
        rqtGeomReferentielEpure,
        rqtPpvCptBdd,
        schema,
        table_graph,
        table_vertex,
        ...
        with ct.ConnexionBdd(localisation) as c:
            ...
            dfDepartementale = gp.read_postgis(
                rqtGeomReferentielEpure, c.sqlAlchemyConn
            )
            ppvIdComptag = pd.read_sql(rqtPpvCptBdd, c.sqlAlchemyConn)

        # dico des troncons elementaire
        simplification = self.troncon_elemntaires(
            schema, table_graph, table_vertex, liste_lignes=list_lgn, id_name="id"
        )
        # plus proche voisin des comptages tournants
        ppvTournIgn = O.plus_proche_voisin(
            dfTournant[["troncon", "geometry"]],
            dfDepartementale,
            20,
            "troncon",
            "id_ign",
        ...
        ppv_final = (
            ppvTournIgn.merge(cpt_existant, on="id_ign", how="left")
            .merge(ppvIdComptag, on="id_comptag", how="left")
            .rename(columns={"id_ign_x": "id_ign_cpt_new", "id_ign_y": "id_ign_lin"})
        )
        # ajout d'un attribut d'identification des correpsondance
        ppv_final["correspondance"] = ppv_final.apply(
            lambda x: pt_corresp(x["id_ign_lin"], x["id_ign_cpt_new"], simplification),
            axis=1,
        )
    ...
    def old_correctionCorrespondanceTournant(
        self, dicoCorrespTourn, tournant, correspTournant
    ):
        """
        ...
        """
        cptTournantAffecte = tournant.merge(
            correspTournant[["troncon", "id_comptag", "correspondance"]],
            on="troncon",
            how="left",
        )
        self.assignerCptInconnus(dicoCorrespTourn, cptTournantAffecte)
        cptTournantAffecte.loc[
            ~cptTournantAffecte.id_comptag.isna(), "correspondance"
        ] = True
        cptTournantAffecte.loc[
            cptTournantAffecte.id_comptag.isna(), "correspondance"
        ] = False
    ...
    def old_creerNouveauPointTournants(self, cptTournantAffecte, dicoNewCpt):
        ...
        """
        cptTournantInconnu = cptTournantAffecte.loc[
            cptTournantAffecte.correspondance == False
        ]
        df_attr_insert = cptTournantInconnu.merge(
            pd.DataFrame(dicoNewCpt).assign(
                src_geo="tableur millesime 2019",
                dep="33",
                reseau="RD",
                gestionnai="CD33",
                concession="N",
                obs_geo="pr et abscisse aproximatifs",
            ),
            on="troncon",
        )
        df_attr_insert["route"] = df_attr_insert.route.apply(
            lambda x: O.epurationNomRoute(x)
        )
        df_attr_insert["id_comptag"] = df_attr_insert.apply(
            lambda x: f"33-{x['route']}-{x['pr']}+{x['absc']}", axis=1
        )
        df_attr_insert["x_l93"] = round(df_attr_insert.geometry.x, 3)
        df_attr_insert["y_l93"] = round(df_attr_insert.geometry.x, 3)
        df_attr_insert[f"obs_{self.annee}"] = (
            "nouveau_point, " + df_attr_insert[f"obs_{self.annee}"]
        )
        return df_attr_insert[
            [
                "id_comptag",
                "route",
                "type_poste",
...
class Comptage_cd79(Comptage):
    ...
    def ouvertureMiseEnForme(self):
        ...
        gdfAnnee["id_comptag"] = gdfAnnee.apply(
            lambda x: f"79-{x.Axe}-{x.PR_dernie_.split('.')[0]}+{int(x.PR_dernie_.split('.')[1])}",
            axis=1,
        )
        gdfAnnee["date_deb"] = gdfAnnee.Semaine.apply(
            lambda x: dt.datetime.strptime(f"2021-W{int(x)}-1", "%G-W%V-%u").strftime(
                "%Y/%m/%d"
            )
        )
        gdfAnnee["date_fin"] = gdfAnnee.Semaine.apply(
            lambda x: dt.datetime.strptime(f"2021-W{int(x)}-7", "%G-W%V-%u").strftime(
                "%Y/%m/%d"
            )
        )
    ...
    def miseAJourTechno(self, dfSource):
        ...
        with ct.ConnexionBdd() as c:
            dfSource.drop("geometry", axis=1).to_sql(
                f"cd79_{self.annee}_temp", c.sqlAlchemyConn, schema="public"
            )
...
class Comptage_vinci(Comptage):
    ...
    def ouvrir_fichier(self):
        donnees_brutes = pd.read_excel(self.fichier_perm).rename(
            columns={"(*) PR début": "pr_deb"}
        )
    ...
    def importer_donnees_correspondance(self):
        with ct.ConnexionBdd(nomConnBddOtv) as c:
            ...
            base = pd.read_sql(rqt, c.sqlAlchemyConn).rename(
                columns={"(*) PR début": "pr_deb"}
            )
    ...
    def comptage_forme(self):
        ...
        self.df_attr = (
            donnees_brutes[
                [
                    "pr_deb",
                    f"TMJA {str(self.annee)}",
                    f"Pc PL {str(self.annee)}",
                    "Vitesse moyenne annuelle",
                ]
            ]
            .merge(base[["pr_deb", "ID"]], on="pr_deb", how="left")
            .rename(
                columns={
                    "ID": "id_comptag",
                    f"TMJA {str(self.annee)}": "tmja",
                    f"Pc PL {str(self.annee)}": "pc_pl",
                    "Vitesse moyenne annuelle": "vmoy",
                }
            )
        )
        ...
        tmjm_mens = (
            donnees_brutes[
                [
                    a
                    for a in donnees_brutes.columns
                    if a == "pr_deb" or "TMJM " + str(self.annee) in a
                ]
            ]
            .merge(base[["pr_deb", "ID"]], on="pr_deb")
            .rename(columns={"ID": "id_comptag"})
            .drop("pr_deb", axis=1)
        )
        tmjm_mens.rename(
            columns={
                c: k
                for c in tmjm_mens.columns
                if c != "id_comptag"
                for k, v in dico_mois.items()
                if v[0] == int(c[-2:])
            },
        ...
        pc_pl_mens = (
            donnees_brutes[
                [
                    a
                    for a in donnees_brutes.columns
                    if a == "pr_deb" or re.search("Pc PL [0-9]{2}$", a)
                ]
            ]
            .merge(base[["pr_deb", "ID"]], on="pr_deb")
            .rename(columns={"ID": "id_comptag"})
        ...
        self.df_attr_mens = pd.concat(
            [tmjm_mens, pc_pl_mens], axis=0, sort=False
        ).sort_values("id_comptag")
    ...
    def update_bdd_Vinci(self, schema, table):
        val_txt = self.creer_valeur_txt_update(
            self.df_attr, ["id_comptag", "tmja", "pc_pl", "src"]
        )
        self.update_bdd(
            schema,
            table,
...
class Comptage_alienor(Comptage):
    ...
    def ouvrir_et_separe_donnees(self):
        ...
        donnees_agregees = donnees[
            [
                a
                for a in donnees.columns
                if a
                in ("id_comptag", "TMJA_" + str(self.annee), "Pc_PL_" + str(self.annee))
            ]
        ].copy()
        donnees_agregees.rename(
            columns={a: a.lower() for a in donnees_agregees.columns}, inplace=True
        )
        donnees_agregees["tmja_" + str(self.annee)] = donnees_agregees[
            "tmja_" + str(self.annee)
        ].apply(lambda x: int(x))
        donnees_agregees["src"] = "tableur"
        donnees_agregees["fichier"] = os.path.basename(self.fichier_perm)
        tmjm_mens = donnees[
            [a for a in donnees.columns if a == "id_comptag" or "TMJM" in a]
        ].copy()  # or re.search('Pc_PL_[0-9]{4}_[0-9]{2}',a)
        tmjm_mens.rename(
            columns={
                c: k
                for c in tmjm_mens.columns
        ...
        pc_pl_mens = donnees[
            [
                a
                for a in donnees.columns
                if a == "id_comptag" or re.search("Pc_PL_[0-9]{4}_[0-9]{2}", a)
            ]
        ].copy()
        pc_pl_mens.rename(
            columns={
    ...
    def update_bdd_Alienor(self, schema, table):
        ...
        self.update_bdd(
            ...
            val_txt,
            {
                "tmja_" + str(self.annee): "tmja_" + str(self.annee),
                "pc_pl_" + str(self.annee): "pc_pl_" + str(self.annee),
...
class Comptage_atlandes(Comptage):
    ...
    def miseEnForme(self):
        ...
        donnees = donnees.loc[
            (~donnees["TMJA 2 sens confondus"].isna()) & (~donnees["TMJM VL"].isna())
        ].copy()
        val_l0 = donnees.iloc[0].values
        col_tmjm_vl = donnees.columns[
            donnees.columns.tolist()
            .index("TMJM VL") : donnees.columns.tolist()
            .index("TMJM PL")
        ]
        dico_nom_tmjm_vl = {
            a: "tmjm_vl_" + b
        ...
        donnees = (
            donnees.rename(columns=dico_nom_tmjm_vl)
            .rename(columns=dico_nom_tmjm_pl)
            .rename(
                columns={"TMJA 2 sens confondus": "tmja_vl", "Unnamed: 8": "tmja_pl"}
            )
        )
        donnees = donnees.iloc[1:].copy()
        donnees["id_comptag"] = donnees.apply(
            lambda x: f"40-A63-{str(x['Echangeur de début de section Trafic'])}+{str(x['Unnamed: 2'])}"
            if x["Echangeur de début de section Trafic"] != 36
            else f"33-A63-{str(x['Echangeur de début de section Trafic'])}+{str(x['Unnamed: 2'])}",
            axis=1,
        )
    ...
    def donneesAgregees(self):
        ...
        """
        donneesAgrege = self.miseEnForme()[
            ["id_comptag", "tmja_vl", "tmja_pl", "fichier", "annee", "src"]
        ].copy()
        donneesAgrege["type_veh"] = "vl/pl"
        donneesAgrege["tmja"] = (donneesAgrege.tmja_vl + donneesAgrege.tmja_pl).astype(
            int
        )
    ...
    def donnees_mens(self):
        ...
        donnees_mens = donnees[
            [a for a in donnees.columns if a in colFixe or "tmjm" in a]
        ].copy()
        # calcul du tmjm_tv
        for m in dico_mois.values():
            donnees_mens[f"tmjm_tv_{m[1]}"] = (
                donnees_mens[f"tmjm_vl_{m[1]}"] + donnees_mens[f"tmjm_pl_{m[1]}"]
            )
            donnees_mens[f"tmjm_pc_pl_{m[1]}"] = donnees_mens.apply(
                lambda x: round(x[f"tmjm_pl_{m[1]}"] / x[f"tmjm_tv_{m[1]}"] * 100, 2),
                axis=1,
            )
        tmjm_tv_mens = donnees_mens[
            [a for a in donnees_mens.columns if "tv" in a or a in colFixe]
        ].copy()
        tmjm_tv_mens.rename(
            columns={
                c: k
                for c in tmjm_tv_mens.columns
                if c != "id_comptag"
                for k, v in dico_mois.items()
                if v[1] == c.split("_")[-1]
            },
        ...
        tmjm_pl_mens = donnees_mens[
            [a for a in donnees_mens.columns if "pc_pl_" in a or a in colFixe]
        ].copy()
        tmjm_pl_mens.rename(
            columns={
                c: k
                for c in tmjm_pl_mens.columns
                if c != "id_comptag"
                for k, v in dico_mois.items()
                if v[1] == c.split("_")[-1]
            },
        ...
        donnees_mens_final = pd.concat(
            [tmjm_tv_mens, tmjm_pl_mens], axis=0, sort=False
        ).sort_values("id_comptag")
...
class Comptage_Cofiroute(Comptage):
    ...
    def insererDonneesComptageBdd(self, inserer=False):
        dfComptage = creer_comptage(
            self.dfFichier.id_comptag, self.annee, "tableau excel", "tv/pl"
        )
        if inserer:
    ...
    def insererDonneesAgregeBdd(self, inserer=False):
        dfAgrege = structureBddOld2NewForm(
            self.dfFichier.rename(
                columns={
                    f"TMJA {self.annee}": "tmja",
                    f"Pc PL {self.annee}": "pc_pl",
                    "Vitesse moyenne annuelle (km/h)": "vmoy",
                }
            ).assign(fichier=os.path.basename(self.fichier), annee=self.annee),
            ["id_comptag", "fichier", "annee"],
            ["tmja", "pc_pl", "vmoy"],
            "agrege",
        )
        if inserer:
    ...
    def insererDonneesMensuelleBdd(self, inserer=False):
        ...
        tmjm_tv_mens = tmjm_tv_mens.rename(
            columns={
                c: k
                for c in tmjm_tv_mens.columns
                if c != "id_comptag"
                for k, v in dico_mois.items()
                if str(v[0]).zfill(2) == c.split(" ")[-1]
            }
        ).assign(donnees_type="tmja")
        tmjm_pl_mens = self.dfFichier[self.colPcplMens + ["id_comptag"]].copy()
        tmjm_pl_mens = tmjm_pl_mens.rename(
            columns={
                c: k
                for c in tmjm_pl_mens.columns
                if c != "id_comptag"
        ...
        ).assign(donnees_type="pc_pl")
        donnees_mens_final = (
            pd.concat([tmjm_tv_mens, tmjm_pl_mens])
            .sort_values("id_comptag")
            .assign(annee=self.annee, fichier=os.path.basename(self.fichier))
        )
        dfMensuel = structureBddOld2NewForm(
            donnees_mens_final,
            ["id_comptag", "annee", "donnees_type", "fichier"],
            list(dico_mois.keys()),
            "mensuel",
        )
        if inserer:
...
class Comptage_GrandPoitiers(Comptage):
    ...
    def donnnees_agregees(self, fichier):
        donnees_agregees = pd.read_excel(
            os.path.join(self.dossier, fichier), sheet_name="Informations"
        )
    ...
    def donnees_horaires(self, fichier, start=8, step=34):
        donnees_detaillees = pd.read_excel(
            os.path.join(self.dossier, fichier), sheet_name="Tableau", header=None
        )
        ...
        for i, j in enumerate(range(start, len(donnees_detaillees), step)):
            ...
            donnees["jour"] = donnees["date"].apply(
                lambda x: pd.to_datetime(x.strftime("%Y-%m-%d"))
            )
            donnees["heure"] = donnees["date"].apply(
                lambda x: f"h{x.hour}_{x.hour+1 if x.hour!=24 else 24}"
            )
            donnees = pd.concat(
                [
                    donnees[["jour", "heure", "tv"]].pivot(
                        index="jour", columns="heure", values="tv"
                    ),
                    donnees[["jour", "heure", "pl"]].pivot(
                        index="jour", columns="heure", values="pl"
                    ),
                ],
                axis=0,
                sort=False,
            ...
            if i == 0:
                ...
                donnees_horaire = pd.concat(
                    [donnees_horaire, donnees], axis=0, sort=False
                )
    ...
    def cumul_sens(self, ref_pt, id_comptag):
        ...
        if len(ref_pt) == 1:
            fichiers_sens = (
                [f for f in self.fichiers if f"Poste {ref_pt[0]} - sens" in f]
                if ref_pt[0] != "20"
                else [f for f in self.fichiers if re.search("Poste 20[A,B] - sens", f)]
            )
        elif len(ref_pt) == 2:
            fichiers_sens = [
                f
                ...
                if (
                    f"Poste {ref_pt[0]} - sens" in f or f"Poste {ref_pt[1]} - sens" in f
                )
            ]
        else:
        ...
        donnees_horaires_f = (
            liste_donnees_horaire[0]
            .set_index("jour")
            .add(liste_donnees_horaire[1].set_index("jour"))
            .reset_index()
            if len(fichiers_sens) > 1
            else liste_donnees_horaire[0]
        )
    ...
    def comptage_forme(self):
        ...
        for p in points_existants.point_comp.tolist():
            if isinstance(
                p, str
            ):  # ne pas prendre en compte les points sans références
                ref_pt = p.split("_")
                id_comptag = points_existants.loc[
                    points_existants["point_comp"] == p
                ].id_comptag.to_numpy()[0]
                try:
                    tmja, pc_pl, donnees_horaires_f, fichiers_sens = self.cumul_sens(
                        ref_pt, id_comptag
                    )
                ...
                dico_cpt[id_comptag] = {
                    "tmja": tmja,
                    "pc_pl": pc_pl,
                    "src": f"tableurs : {', '.join(fichiers_sens)}",
                }
                if i == 0:
                    donnees_horaires_tot = donnees_horaires_f.copy()
                else:
                    donnees_horaires_tot = pd.concat(
                        [donnees_horaires_tot, donnees_horaires_f], axis=0, sort=False
                    )
                i += 1
        self.df_attr = (
            pd.DataFrame.from_dict(dico_cpt, orient="index")
            .reset_index()
            .rename(columns={"index": "id_comptag"})
        )
    ...
    class PasDeFichierError(Exception):
        def __init__(self, ref_pt):
            Exception.__init__(
                self, f"pas de fihcier pour les postes de comptage {ref_pt} "
            )
...
class Comptage_Niort(Comptage):
    ...
    def creer_dico(self, dico_id_comptag, dicoFormat):
        ...
        for v in dicoFormat.values():
            if v["format"] not in niort_formatFichierAccepte:
                raise FormatError(
                    f"le format {v['format']} n'est pas pris en charge dans le code"
                )
        for chemin, dossier, files in os.walk(self.dossier):
            for d in dossier:
                if d not in dicoFormat:
                    warnings.warn(
                        f"le dossier {d} n'est pas présent dans le dico des format. A ajouter avant nouvelle itération"
                    )
                else:
                    dico_fichier[d] = [
                        chemin + os.sep + d + os.sep + f
                        for f in O.ListerFichierDossier(
        ...
        for k, v in dico_fichier.items():
            if len(v) > 2:
                raise NotImplementedError(
                    f"""le dossier {k} contient plus de 2 comptages. 
    ...
    def formater_fichier_csv(self, fichier):
        ...
        if (
            nb_jours_mesure == 8
        ):  # si le nombre total de jours est inférieur à 9, il faudra regrouper les jours de début et de fin pour avoir une journée complete, ou alors c'est qu'une erreur a ete commise lors de la mesure de trafic (duree < 7 jours)
            df_fichier.loc[
                df_fichier.DateTime.apply(lambda x: x.dayofyear == jourMin), "DateTime"
            ] = df_fichier.loc[
                df_fichier.DateTime.apply(lambda x: x.dayofyear == jourMin)
            ].DateTime.apply(
                lambda x: x + pd.Timedelta("7D")
            )
        elif nb_jours_mesure > 8:
            df_fichier = df_fichier.loc[
                df_fichier.DateTime.apply(
                    lambda x: (x.dayofyear != jourMin) & (x.dayofyear != jourMax)
                )
            ].copy()
    ...
    def csv_identifier_type_veh(self, df_brute):
        ...
        """
        vl = df_brute.loc[
            (df_brute["Length"] < 7.2) & (df_brute["AdviceCode"] != 128)
        ].copy()
        pl = df_brute.loc[
            (df_brute["Length"] >= 7.2)
            & (df_brute["Length"] < 22)
            & (df_brute["AdviceCode"] != 128)
        ].copy()
        return vl, pl
    ...
    def csv_agrege_donnees_brutes(self, vl, pl):
        ...
        """
        vl_agrege_h = (
            vl.set_index("DateTime")
            .resample("H")
            .AdviceCode.count()
            .reset_index()
            .rename(columns={"AdviceCode": "nb_veh"})
        )
        vl_agrege_j = (
            vl.set_index("DateTime")
            .resample("D")
    ...
    def csv_indicateurs_agrege(self, vl_agrege_j, pl_agrege_j):
        ...
        tmjo = round(
            vl_agrege_j.loc[
                vl_agrege_j.DateTime.dt.dayofweek.isin(range(5))
            ].nb_veh.mean()
            + pl_agrege_j.loc[
                pl_agrege_j.DateTime.dt.dayofweek.isin(range(5))
            ].nb_veh.mean()
        )
        nb_pl_o = pl_agrege_j.loc[
            pl_agrege_j.DateTime.dt.dayofweek.isin(range(5))
        ].nb_veh.mean()
    ...
    def csv_donnees_horaires(self, vl_agrege_h, pl_agrege_h):
        ...
        """
        vl_agrege_h["jour"] = vl_agrege_h["DateTime"].apply(
            lambda x: pd.to_datetime(x.strftime("%Y-%m-%d"))
        )
        vl_agrege_h["heure"] = vl_agrege_h["DateTime"].apply(
            lambda x: f"h{x.hour}_{x.hour+1 if x.hour != 24 else 24}"
        )
        pl_agrege_h["jour"] = pl_agrege_h["DateTime"].apply(
            lambda x: pd.to_datetime(x.strftime("%Y-%m-%d"))
        )
        pl_agrege_h["heure"] = pl_agrege_h["DateTime"].apply(
            lambda x: f"h{x.hour}_{x.hour+1 if x.hour != 24 else 24}"
        )

        pl_agrege_h = pl_agrege_h[["jour", "heure", "nb_veh"]].pivot(
            index="jour", columns="heure", values="nb_veh"
        )
        calcul_agreg_h = pd.concat(
            [
                vl_agrege_h[["jour", "heure", "nb_veh"]].pivot(
                    index="jour", columns="heure", values="nb_veh"
                ),
                pl_agrege_h,
            ],
        ...
        donnees_horaires = (
            pd.concat([tv_agrege_h, pl_agrege_h], axis=0, sort=False)
            .fillna(0)
            .reset_index()
        )
    ...
    def csv_indic_agreg_2sens(self, id_comptag, fichiers):
        ...
        for f in fichiers:
            try:
                ...
                return pd.DataFrame([])
        tmja, nb_pl, tmjo, nb_pl_o, dateMin, dateMax = (
            [list_resultat[i][2] for i in range(len(fichiers))],
        ...
        periode = (
            f"{min(dateMin).strftime('%Y/%m/%d')}-{max(dateMax).strftime('%Y/%m/%d')}"
        )
        return gp.GeoDataFrame(
            {
                ...
                "tmjo": tmjo,
                "pc_pl_o": pc_pl_o,
                "geometry": geomFromIdComptagCommunal(id_comptag),
    ...
    def traiter_csv_sens_unique(self, fichier):
        ...
        (
            ...
            pl_agrege_h,
            pl_agrege_j,
        ) = self.csv_agrege_donnees_brutes(vl, pl)
        nb_vl, nb_pl, tmja, pc_pl, tmjo, pc_pl_o, nb_pl_o = self.csv_indicateurs_agrege(
            vl_agrege_j, pl_agrege_j
        )
    ...
    def xlsCpevAgrege(self, fichier, id_comptag):
        ...
        periode = "-".join(
            [
                pd.to_datetime(e, dayfirst=True).strftime("%Y/%m/%d")
                for e in re.findall(
                    "[0-9]{1,2}/[0-1][0-9]/20[0-2][0-9]", dfBrute.iloc[1, 16]
                )
            ]
        )
    ...
    def horaire_2_sens(self, id_comptag, list_df_sens):
        ...
        comparer2Sens(
            df_finale.assign(id_comptag=id_comptag),
            attributSens="sens",
            attributIndicateur="type_veh",
            facteurComp=10000,
            TauxErreur=0.00001,
        )
    ...
    def xlsCpevHoraireDebit(self, fichier):
        ...
        dfBruteHoraire = pd.concat(
            [
                dfBrute.iloc[
                    niort_ligneDebutDebitHoraireTv : niort_ligneDebutDebitHoraireTv
                    + niort_nbJoursHoraireCpev,
                    niort_colonneDebutDebitHoraire:niort_colonneFinDebitHoraire,
                ].assign(type_veh="TV"),
                dfBrute.iloc[
                    niort_ligneDebutDebitHorairePl : niort_ligneDebutDebitHorairePl
                    + niort_nbJoursHoraireCpev,
                    niort_colonneDebutDebitHoraire:niort_colonneFinDebitHoraire,
                ].assign(type_veh="PL"),
            ]
        ...
        dfBruteHoraire["jour"] = dfBruteHoraire.jour.apply(
            lambda x: pd.to_datetime(f"{x.split('.')[1]}/{self.annee}", dayfirst=True)
        )
    ...
    def xlsCpevHoraireVitesse(self, fichier):
        ...
        dfBrute = pd.read_excel(fichier)
        for e in range(
            niort_vmoyHoraireVlStartCpev,
            niort_vmoyHoraireVlStartCpev
            + (niort_vmoyHoraireVlPasCpev * niort_nbJoursHoraireCpev),
            niort_vmoyHoraireVlPasCpev,
        ):
            dfJourVmoyHoraireVl = dfBrute.iloc[
                e : e + 24, [niort_colonneVmoyHoraire]
            ].T.copy()
            dfJourVmoyHorairePl = dfBrute.iloc[
                e + niort_vmoyHorairePlPasCpev : e + niort_vmoyHorairePlPasCpev + 24,
                [niort_colonneVmoyHoraire],
            ].T.copy()
            ...
            listVmoyHoraire.append(
                pd.concat([dfJourVmoyHoraireVl, dfJourVmoyHorairePl]).assign(
                    jour=pd.to_datetime(
                        dfBrute.iloc[e + 1, niort_colonneVmoyHoraireJour], dayfirst=True
                    )
                )
            )
    ...
    def xlsCpevHoraire(self, fichier, id_comptag):
        return (
            pd.concat(
                [self.xlsCpevHoraireDebit(fichier), self.xlsCpevHoraireVitesse(fichier)]
            )
            .assign(fichier=os.path.basename(fichier), id_comptag=id_comptag)
            .reset_index(drop=True)
        )

    def calculAgregeHoraireTouteSource(
        self, dico_fichiers_final, dico_id_comptag, dicoFormat
    ):
        """
        ...
        for k, v in dico_fichiers_final.items():
            ...
            if dicoFormat[doss]["type"] == "horaire":
                ...
                if dicoFormat[doss]["format"] == "xls":
                    listGdfAgrege.append(
                        self.xlsCpevAgrege(v[0], k).assign(src=src, annee=self.annee)
                    )
                    listDfHoraire.append(
                        self.xlsCpevHoraire(v[0], k).assign(annee=self.annee)
                    )
                elif dicoFormat[doss]["format"] == "csv":
                    for i, f in enumerate(v, 1):
                        list_2sens.append(
                            self.traiter_csv_sens_unique(f)[7].assign(
                                sens=f"sens {i}",
                    ...
                    if not list_2sens:
                        warnings.warn(
                            f"des donnees sont vides pour le comptage {k} dans le dossier {doss}"
                        )
                        list_2sens = []
                        continue
                    listDfHoraire.append(
                        self.horaire_2_sens(k, list_2sens).assign(
                            fichier=os.path.basename(v[0]), annee=self.annee
                        )
                    )
                    listGdfAgrege.append(
                        gp.GeoDataFrame(
                            self.csv_indic_agreg_2sens(k, v).assign(
                                src=src, annee=self.annee
                            ),
                            geometry=[geomFromIdComptagCommunal(k)],
                            crs="EPSG:2154",
                        )
                    )
                    list_2sens = []
                    ...
                    if doss[:5].lower() == "gare_":
                        ...
                        numSensAUtiliser = (
                            [intSensAUtiliser]
                            if intSensAUtiliser <= 10
                            else [
                                int(str(intSensAUtiliser)[0]),
                                int(str(intSensAUtiliser)[1]),
                            ]
                        )
                        ...
                        cpt = MHCorbin(v[0])
                    dfHoraire = (
                        cpt.formaterDonneesHoraires(
                            cpt.formaterDonneesIndiv(cpt.dfAgreg2Sens)
                        )
                        .assign(id_comptag=k)
                        .reset_index()
                        .assign(annee=self.annee, fichier=os.path.basename(v[0]))
                    )
                    dfAgrege = gp.GeoDataFrame(
                        tmjaDepuisHoraire(dfHoraire)
                        .pivot(
                            index=["id_comptag", "annee", "fichier"],
                            columns="indicateur",
                            values="valeur",
                        )
                        .reset_index()
                        .assign(
                            periode=periodeDepuisHoraire(dfHoraire).periode[0],
                            src="horaire_mdb",
                        ),
                        geometry=[geomFromIdComptagCommunal(k)],
                        crs="EPSG:2154",
        ...
        return pd.concat(listGdfAgrege).reset_index(drop=True), pd.concat(
            listDfHoraire
        ).reset_index(drop=True)
    ...
    def miseEnFormeCompteur(self, dfAgregeFinale, listRd, listSensUnique):
        ...
        return creerCompteur(
            dfAgregeFinale.assign(
                type_poste="ponctuel",
                src_geo="carto_gestionnaire",
                pr=None,
                abs=None,
                route=dfAgregeFinale.id_comptag.apply(
                    lambda x: re.sub(
                        "[0-9]{1,3}[a-z]{0,1}_", "", x.split("-")[1]
                    ).replace("_", " ")
                ),
                src_cpt=dfAgregeFinale.id_comptag.apply(
                    lambda x: "collectivite territoriale"
                    if x in listRd
                    else "convention gestionnaire"
                ),
                convention=True,
            ...
            "geometry",
            79,
            dfAgregeFinale.id_comptag.apply(lambda x: "RD" if x in listRd else "VC"),
            dfAgregeFinale.id_comptag.apply(
                lambda x: "CD79" if x in listRd else "Niort"
            ),
            False,
        )
...
class Comptage_GrandDax(Comptage):
    def __init__(self, fichiers_pt_comptages):
        ...
        """
        self.fichiers_pt_comptages = gp.read_file(
            fichiers_pt_comptages, encoding="UTF-8"
        )
    ...
    def MaJ_fichiers_pt_comptages(self):
        ...
        """
        self.fichiers_pt_comptages[
            "x_wgs84"
        ] = self.fichiers_pt_comptages.geometry.to_crs(
            {"proj": "longlat", "ellps": "WGS84", "datum": "WGS84"}
        ).apply(
            lambda x: round(x.x, 4)
        )
        self.fichiers_pt_comptages[
            "y_wgs84"
        ] = self.fichiers_pt_comptages.geometry.to_crs(
            ...
            lambda x: round(x.y, 4)
        )
        self.fichiers_pt_comptages["id_comptag"] = self.fichiers_pt_comptages.apply(
            lambda x: f"GrdDax-{x.nom_rte}-{str(x.x_wgs84)};{str(x.y_wgs84)}", axis=1
        )
        ...
        self.fichiers_pt_comptages["x_l93"] = self.fichiers_pt_comptages.geometry.apply(
            lambda x: round(x.x, 3)
        )
        self.fichiers_pt_comptages["y_l93"] = self.fichiers_pt_comptages.geometry.apply(
            lambda x: round(x.y, 3)
        )

    def ouvrir_feuille_infos(self, dossier, fichier):
        donnees_generale = pd.read_excel(
            os.path.join(dossier, fichier), sheet_name="Informations"
        )
        donnees_gal_sansNa = donnees_generale.dropna(how="all").dropna(
            axis=1, how="all"
        )
    ...
    def creer_df_agrege(self):
        ...
        for dossier, fichiers, id_comptag in zip(
            self.fichiers_pt_comptages.dossier.tolist(),
            self.fichiers_pt_comptages.nom_fich.tolist(),
            ...
            if (
                fichiers == None
            ):  # si pas de fichiers renseigne normelent seulement 2 IFX dans le dossier
            ...
            donnees_agregees = [
                self.recup_donnees_agregees(self.ouvrir_feuille_infos(dossier, f)[1])[
                    6:
                ]
                for f in fichiers
            ]
            ...
            for a in range(2011, 2021):
                if int(annee) == a:
                    ...
                    dico[f"src_{a}"].append(
                        ";".join([os.path.join(dossier, f) for f in fichiers])
                    )
                else:
        ...
        self.df_attr = self.fichiers_pt_comptages[
            [
                "id_comptag",
                "geometry",
                "dep",
        ...
        ].merge(pd.DataFrame.from_dict(dico), on="id_comptag")

    def creer_donnees_source(self, dossier, fichier, date_debut, duree_cpt):
        return [
            (
                pd.read_excel(
                    os.path.join(dossier, fichier), sheet_name=f"Tableau {i+1}"
                ),
                date_debut + pd.Timedelta(f"{i}D"),
            )
            for i in range(duree_cpt)
        ]

    def creer_df_horaire_brutes(self, donnees_horaires_brutes, date):  # pour 1 journee
        donnees = (
            [donnees_horaires_brutes, "VL", "Unnamed: 29"],
            [donnees_horaires_brutes, "PL", "Unnamed: 30"],
        )

        def df_horaire_brute(
            donnees_horaires_brutes, type_veh, attr
        ):  # pourcahque type de veh
            test = donnees_horaires_brutes.loc[
                15:38, ["TABLEAU HORAIRE DE SYNTHESE DES RESULTATS JOURNALIERS", attr]
            ].copy()
            test["type_veh"] = type_veh
            test.rename(
                columns={
                    "TABLEAU HORAIRE DE SYNTHESE DES RESULTATS JOURNALIERS": "heure",
                    attr: "nb_veh",
                },
                inplace=True,
            )
        ...
        test = pd.concat(
            [df_horaire_brute(*d) for d in donnees], axis=0, sort=False
        ).reset_index(drop=True)
        df_horaire = pd.concat(
            [
                test.loc[test.type_veh == v[1]].pivot(
                    index="type_veh", columns="heure", values="nb_veh"
                )
                for v in donnees
            ],
            axis=0,
    ...
    def creer_df_horaire_tt_jour(self, donnees_tout_jour):
        return pd.concat(
            [self.creer_df_horaire_brutes(*d) for d in donnees_tout_jour],
            axis=0,
            sort=False,
        )
    ...
    def creer_df_horaire_2_sens(self, dossier, liste_fichiers, id_comptag):
        ...
        date_debut, date_fin, duree_cpt, annee = self.recup_infos(
            donnees_gal_sansNa, 10, 11
        )

        donnees_tout_jour = (
            self.creer_donnees_source(dossier, f, date_debut, duree_cpt)
            for f in liste_fichiers
        )
        df_2_sens = pd.concat(
            [self.creer_df_horaire_tt_jour(d) for d in donnees_tout_jour],
            axis=0,
            sort=False,
        )
        ...
        df_2_sens.rename(
            columns={
                k: f"h{k.replace('H','').replace('-','_')}"
                for k in df_2_sens.columns
                if k[-1] == "H"
            },
            inplace=True,
        )
    ...
    def df_horaire_final(self):
        self.df_attr_horaire = pd.concat(
            [
                self.creer_df_horaire_2_sens(dossier, fichiers, id_comptag)
                for dossier, fichiers, id_comptag in zip(
                    self.fichiers_pt_comptages.dossier.tolist(),
                    self.fichiers_pt_comptages.nom_fich.tolist(),
                    self.fichiers_pt_comptages.id_comptag.tolist(),
...
class Comptage_Limoges_Metropole(Comptage):
    def __init__(self, fichiers_shape, fichier_zone):
        ...
        self.df_trafic = df_trafic.loc[
            (df_trafic.geometry != None)
            & (~df_trafic.tv_moyjour.isna())
            & (df_trafic["type_capt"] != "Manuel")
        ].copy()
        self.fichier_zone = fichier_zone
        self.df_trafic["geom_wgs84"] = self.df_trafic.geometry.to_crs(epsg="4326")
        self.df_trafic["geom_wgs84_x"] = self.df_trafic.geom_wgs84.apply(
            lambda x: str(round(x.x, 4))
        )
        self.df_trafic["geom_wgs84_y"] = self.df_trafic.geom_wgs84.apply(
            lambda x: str(round(x.y, 4))
        )
    ...
    def isoler_zone(self, num_zone):
        ...
        """
        zone_cbl = (
            self.df_trafic.loc[self.df_trafic["id_groupe"] == num_zone][
                [
                    "type",
                    "geometry",
                    "date_deb",
    ...
    def calcul_attributs_commun(
        self, format_cpt, geom, nom_voie, wgs84_geom_x, wgs84_geom_y, num_zone
    ):
        # geometry
        ...
        format_cpt["id_comptag"] = (
            f"LimMet-{nom_voie.lower()}-{wgs84_geom_x};{wgs84_geom_y}"
            if nom_voie
            else f"LimMet-??-{wgs84_geom_x};{wgs84_geom_y}"
        )

    def simplifier1PtComptage(
        self, zone_cbl, geom, wgs84_geom_x, wgs84_geom_y, num_zone, typeCpt
    ):
        """
        ...
        dbl_max = dbl.loc[
            dbl["tv_moyjour"] == dbl.groupby("annee").tv_moyjour.transform(max)
        ].copy()
        dbl_max["obs"] = dbl_max.apply(
            lambda x: conversion_date(x["date_deb"], x["date_fin"]), axis=1
        )
        format_cpt = dbl_max[
            ["type", "annee", "tv_moyjour", "pl_pourcen", "id_groupe", "obs"]
        ].pivot(
            index="id_groupe",
            columns="annee",
        ...
        colonnes_name = (
            ["tmja_" + a for a in annees]
            + ["pc_pl_" + a for a in annees]
            + ["obs_" + a for a in annees]
        )
        ...
        # nom de voies
        nom_voie = (
            zone_cbl.loc[zone_cbl["type"] == typeCpt]
            .apply(
                lambda x: x.nom_voie if not pd.isnull(x.nom_voie) else x.position,
                axis=1,
            )
            .unique()[0]
        )
        self.calcul_attributs_commun(
            format_cpt, geom, nom_voie, wgs84_geom_x, wgs84_geom_y, num_zone
        )
        # regrouper les années antérieure à 2010 dans une colonne 'autre'
        dico_toutes_annee_autre = {
            a: {
                "tmja": format_cpt[f"tmja_{a}"].values[0],
                "pc_pl": format_cpt[f"pc_pl_{a}"].values[0],
                "obs": format_cpt[f"obs_{a}"].values[0],
                "src": format_cpt[f"src_{a}"].values[0],
            }
            for a in annees
            if a < "2010"
        }
        if dico_toutes_annee_autre:
            dico_annee_autre_max = dico_toutes_annee_autre[
                max(dico_toutes_annee_autre.keys())
            ]
            ...
            format_cpt.drop(
                [
                    f"{c}_{a}"
                    for c in ("tmja", "pc_pl", "obs", "src")
                    for a in dico_toutes_annee_autre.keys()
                ],
                axis=1,
                inplace=True,
            )
    ...
    def df_intermediaire(
        self, zone_cbl, zone_cbl_grp_type, carac_geom_type, reprojection, num_zone
    ):
        """
        ...
        if "Double" in carac_geom_type.index:
            geom, wgs84_geom_x, wgs84_geom_y = self.creer_geom(
                "Double", carac_geom_type, zone_cbl_grp_type, zone_cbl, reprojection
            )
            df = self.simplifier1PtComptage(
                zone_cbl, geom, wgs84_geom_x, wgs84_geom_y, num_zone, "Double"
            )
            df["obs_supl"] = df["obs_supl"] + " ; comptage 2 sens"
        elif (zone_cbl["type"] == "Simple").all():
            geom, wgs84_geom_x, wgs84_geom_y = self.creer_geom(
                "Simple", carac_geom_type, zone_cbl_grp_type, zone_cbl, reprojection
            )
            if carac_geom_type["Simple"] == 1:
                df = self.simplifier1PtComptage(
                    zone_cbl, geom, wgs84_geom_x, wgs84_geom_y, num_zone, "Simple"
                )
                df["obs_supl"] = df["obs_supl"] + " ; comptage 1 sens"
            else:
                if carac_geom_type["Simple"] == 2:
                    zone_cbl = (
                        zone_cbl.loc[zone_cbl["type"] == "Simple"][
                            [
                                "type",
                                "geometry",
                                "date_deb",
                        ...
                        .merge(
                            zone_cbl.loc[zone_cbl["type"] == "Simple"]
                            .groupby("annee")["pl_pourcen"]
                            .mean(),
                            left_on="annee",
                            right_index=True,
                        )
                        .drop_duplicates(["annee", "tv_moyjour"])
                    )
                    df = self.simplifier1PtComptage(
                        zone_cbl, geom, wgs84_geom_x, wgs84_geom_y, num_zone, "Simple"
                    )
            ...
            if carac_geom_type["DETECTEUR_PC"] == 1:
                df = self.simplifier1PtComptage(
                    zone_cbl, geom, wgs84_geom_x, wgs84_geom_y, num_zone, "DETECTEUR_PC"
                )
                df["obs_supl"] = df["obs_supl"] + " ; comptage 1 sens"
            else:
                raise ValueError("cas non traite")
            ...
            geom, wgs84_geom_x, wgs84_geom_y = self.creer_geom(
                "Mix", carac_geom_type, zone_cbl_grp_type, zone_cbl, reprojection
            )
            if len(zone_cbl) == 1:
                ...
                format_cpt.rename(
                    columns={
                        "tv_moyjour": f"tmja_{annee}",
                        "pl_pourcen": f"pc_pl_{annee}",
                    },
                    inplace=True,
                )
                df = self.calcul_attributs_commun(
                    format_cpt,
                    geom,
                    zone_cbl.nom_voie.values[0],
                    wgs84_geom_x,
                    wgs84_geom_y,
                    num_zone,
                )
    ...
    def creer_geom(
        self, cas, carac_geom_type, zone_cbl_grp_type, zone_cbl, reprojection
    ):
        """
        ...
        if (
            cas == "Double"
            or (cas in ("Simple", "DETECTEUR_PC") and carac_geom_type[cas] == 1)
        ...
        elif (cas == "Simple" and carac_geom_type[cas] == 2) or (
            cas == "Mix" and carac_geom_type["Simple"] == 2
        ):
    ...
    def df_regroupee(self):
        ...
        for i in self.df_trafic.id_groupe.unique():
            ...
            if zone_cbl.empty:
                continue
            list_dfs.append(
                self.df_intermediaire(
                    zone_cbl, zone_cbl_grp_type, carac_geom_type, reprojection, i
                )
            )
        return list_dfs
    ...
    def df_regroupee_complete(self, list_dfs):
        ...
        def type_compteur(ligne, list_colonnes):
            ...
            if all([f"tmja_{annee}" in list_colonnes for annee in date_range]) and all(
                [
                    not pd.isnull(ligne[f"tmja_{annee}"])
                    for annee in (str(i) for i in date_range)
                ]
            ):
                return "permanent"
                ...
                for col in [
                    f"tmja_{annee}"
                    for annee in date_range
                    if f"tmja_{annee}" in list_colonnes
                ]:
        ...
        self.df_attr = gdf.assign(
            dep="87",
            reseau="VC",
            gestionnai="Limoges Metropole",
            concession="N",
            x_l93=lambda x: round(x.geometry.x, 3),
            y_l93=lambda x: round(x.geometry.y, 3),
        )
...
class Comptage_LaRochelle(Comptage):
    ...
    def extraireDonneesSterela(self, fichier):
        ...
        index_date = pd.date_range(
            start=pd.to_datetime(periode.split(" au ")[0][3:], dayfirst=True),
            end=pd.to_datetime(periode.split(" au ")[1], dayfirst=True),
        )
    ...
    def sommer2sens(self, id_comptag, id_cpt):
        ...
        Donnees2Sens = list(
            zip(
                *[
                    self.extraireDonneesSterela(fichier)
                    for fichier in [
                        f"SEMAINE {i}{os.sep}{a.replace('SEMAINE 1',f'SEMAINE {i}')}"
                        for a in id_cpt.split(";")
                        for i in range(1, 4)
                    ]
                ]
            )
        ...
        df_tv = (
            pd.concat([*Donnees2Sens[3]], axis=0, sort=False)
            .groupby("jour")
            .sum()
            .assign(type_veh="TV", id_comptag=id_comptag)
            .reset_index()
        )
        df_pl = (
            pd.concat([*Donnees2Sens[4]], axis=0, sort=False)
            .groupby("jour")
            .sum()
            .assign(type_veh="PL", id_comptag=id_comptag)
            .reset_index()
        )
        ...
        self.df_attr = pd.DataFrame(
            {
                "id_comptag": list_id_comptag,
                "tmja_2015": list_tmja,
                "pc_pl_2015": list_pc_pl,
                "src_2015": "agglo LaRochelle",
    ...
    def update_bdd_LaRochelle(self, schema, table):
        valeurs_txt = self.creer_valeur_txt_update(
            self.df_attr,
            ["id_comptag", "tmja_2015", "pc_pl_2015", "src_2015", "fichier"],
        )
        dico_attr_modif = {
            f"tmja_2016": "tmja_2015",
            f"pc_pl_2016": "pc_pl_2015",
            f"src_2016": "src_2015",
            "fichier": "fichier",
        }
        self.update_bdd(schema, table, valeurs_txt, dico_attr_modif)
...
class Comptage_Anglet(Comptage):
    ...
    def __init__(self, annee, dossier, dossierResume):
        ...
        self.resumeComptageGeoloc = os.path.join(
            dossierResume, f"Comptage{annee}_geoloc.csv"
        )
    ...
    def exporterCsvAGeocoder(self):
        ...
        for files in os.listdir(self.dossier):
            if os.path.isfile(os.path.join(self.dossier, files)) and files.endswith(
                ".mdb"
            ):
                dicoAdresse["fichier"].append(os.path.join(self.dossier, files))
                dicoAdresse["adresse"].append(
                    re.sub("^[1-9]+\.", replaceAdresse, files[:-4])
                )
    ...
    def creerDfGeoloc(self):
        ...
        gdfGeoloc = gp.read_file(
            self.resumeComptageGeoloc, encoding="utf-8"
        )  # C:\Users\martin.schoreisz\Documents\temp\OTV\Anglet\Comptage2019_geoloc.csv
        gdfGeoloc.geometry = gp.points_from_xy(
            x=gdfGeoloc["longitude"], y=gdfGeoloc["latitude"], crs=4326
        )
        gdfGeoloc = gdfGeoloc.to_crs("epsg:2154")
        gdfGeoloc["id_comptag"] = gdfGeoloc.apply(
            lambda x: "Anglet-"
            + re.sub("_{2,50}", "_", re.sub("(,|\.|'| +)", "_", x["adresse"]))
            + f"-{round(float(x['longitude']), 4)};{round(float(x['latitude']),4)}",
            axis=1,
        )
        gdfGeoloc["route"] = gdfGeoloc.adresse.apply(
            lambda x: re.sub("^([1-9]{1,3},)+", "", x)
        )
        gdfGeoloc["reseau"] = "VC"
        gdfGeoloc["dep"] = "64"
    ...
    def indicsTousFichiers(self):
        ...
        for row in self.gdfGeoloc.itertuples(index=False):
            ...
            try:
                ...
                continue
            listDfHoraires.append(
                mhc.formaterDonneesHoraires(mhc.formaterDonneesIndiv(mhc.dfAgreg2Sens))
                .reset_index()
                .assign(
        ...
        self.dfIndicAgrege = tmjaDepuisHoraire(
            self.dfHoraire.assign(annee=self.annee)
        ).merge(
            self.dfHoraire[
                [
                    "id_comptag",
    ...
    def plusProcheVoisinBddRegroupe(self, tableLinauto, distance=15):
        ...
        self.insert_bdd(
            "public",
            f"cpt_anglet_{self.annee}",
            self.gdfGeoloc.drop("fichier", axis=1),
            if_exists="replace",
        )
        rqt = f"""WITH 
                cpteur_base AS (
        ...
        with ct.ConnexionBdd(nomConnBddOtv) as c:
            dfPPV = (
                gp.read_postgis(rqt, c.sqlAlchemyConn, crs="epsg:2154")
                .merge(self.dfIndicAgrege, on="id_comptag", how="left")
                .merge(
                    periodeDepuisHoraire(self.dfHoraire.assign(annee=self.annee)),
                    how="left",
                    on="id_comptag",
                )
            )
    ...
    def isolerComptagesDoublons(self, dfPPV):
        ...
        """
        return dfPPV.loc[
            (dfPPV.duplicated(["gid", "indicateur"], keep=False)) & (~dfPPV.gid.isna())
        ]
    ...
    def cptSsDblEtSsGeom(self, dfPPV, dicoAssoc):
        ...
        """
        dfPPVFiltreAconserver = dfPPV.loc[
            ~dfPPV.id_comptag.isin([v for e in dicoAssoc.values() for v in e])
            & (dfPPV.indicateur == "tmja")
            & (~dfPPV.gid.isna())
        ]
        if not (dfPPVFiltreAconserver.gid.value_counts() == 1).all():
            raise ComptageMultipleSurTronconHomogeneError(
                list(
                    dfPPVFiltreAconserver.loc[
                        dfPPVFiltreAconserver.gid.isin(
                            dfPPVFiltreAconserver.gid.value_counts() != 1
                        )
                    ].gid.unique()
                )
            )
        dfPPVCptRef = dfPPV.loc[
            dfPPV.id_comptag.isin(
                dfPPVFiltreAconserver.id_comptag.tolist()
                + dfPPV.loc[dfPPV.gid.isna()].id_comptag.tolist()
            )
        ].copy()
        dfPPVCptAssoc = dfPPV.loc[
            dfPPV.id_comptag.isin([v for e in dicoAssoc.values() for v in e])
        ].merge(
            pd.DataFrame.from_dict(dicoAssoc, orient="index")
            .reset_index()
            .rename(columns={"index": "cpt_ref", 0: "cpt_assoc"}),
            left_on="id_comptag",
            right_on="cpt_assoc",
        )
    ...
    def insererComptage(self, dfPPVSsDbl, schema="comptage", table="comptage"):
        ...
        dfPPVSsDblTemp["id_comptag_final"] = dfPPVSsDblTemp.apply(
            lambda x: x.id_comptag_bdd
            if not pd.isnull(x.id_comptag_bdd)
            else x.id_comptag,
            axis=1,
        )
        comptage = (
            dfPPVSsDblTemp[["id_comptag_final", "annee", "periode"]]
            .assign(src="gestionnaire", type_veh="tv/pl")
            .rename(columns={"id_comptag_final": "id_comptag"})
            .drop_duplicates()
        )
        # inserer
    ...
    def insererIndics(
        ...
        dfPPVSsDbl,
        schema="comptage",
        tableAgrege="indic_agrege",
        tableHoraire="indic_horaire",
    ):
        """
        ...
        indicAgrege = dfPPVSsDblTemp.merge(
            self.recupererIdUniqComptage(
                dfPPVSsDblTemp.id_comptag_final.tolist(), self.annee
            ),
            left_on=["id_comptag_final", "annee"],
            right_on=["id_comptag", "annee"],
        )[["id_comptag_uniq", "indicateur", "valeur", "fichier"]]
        dfHoraireSsDbl = self.dfHoraire.merge(
            dfPPVSsDblTemp[["id_comptag", "id_comptag_final"]], on="id_comptag"
        )
        indicHoraire = (
            dfHoraireSsDbl.assign(
                annee=self.annee, indicateur=dfHoraireSsDbl.indicateur.str.upper()
            )
            .merge(
                self.recupererIdUniqComptage(
                    dfHoraireSsDbl.id_comptag_final.tolist(), self.annee
                ).rename(columns={"id_comptag": "id_comptag_final"}),
                on=["id_comptag_final", "annee"],
            )
            .drop(
                [
                    "sens_cpt",
    ...
    def insererDatas(
        ...
        dfPPVSsDbl,
        schema="comptage",
        tableComptag="comptage",
        tableCpteur="compteur",
        tableAgrege="indic_agrege",
        tableHoraire="indic_horaire",
    ):
        """
    ...
    def insererQualiteFaible(
        self, dfPPVCptRef, schema="qualite", table="qualite_manuelle_cptag"
    ):
        """
        ...
        O.checkAttributsinDf(
            dfPPVCptRef, ["id_comptag_bdd", "id_comptag", "note_manuelle", "obs"]
        )
        dfQualiteFaible = dfPPVCptRef.loc[dfPPVCptRef.note_manuelle == 1].copy()
        # y associer un id_comptage_final
        dfQualiteFaible["id_comptag_final"] = dfQualiteFaible.apply(
            lambda x: x.id_comptag_bdd
            if not pd.isnull(x.id_comptag_bdd)
            else x.id_comptag,
            axis=1,
        ...
        dfQualiteFaibleAInsere = dfQualiteFaible.merge(
            self.recupererIdUniqComptage(
                dfQualiteFaible.id_comptag_final.tolist(), self.annee
            ),
            left_on="id_comptag_final",
            right_on="id_comptag",
        )[["id_comptag_uniq", "note_manuelle", "obs"]].drop_duplicates(
            ["id_comptag_uniq", "note_manuelle", "obs"]
        )
    ...
    def creerComptageAssoc(self, dfPPVCptAssoc):
        ...
        dfPPVCptAssocInterne["id_comptag_final"] = dfPPVCptAssocInterne.apply(
            lambda x: x.id_comptag_bdd
            if not pd.isnull(x.id_comptag_bdd)
            else x.cpt_ref,
            axis=1,
        ...
        dfAssoc = dfPPVCptAssocInterne.merge(
            self.recupererIdUniqComptage(
                dfPPVCptAssocInterne.id_comptag_final.tolist(), self.annee
            ),
            left_on="id_comptag_final",
            right_on="id_comptag",
        )
        # ajouter les colonnes manquantes
        dfAssocInsert = dfAssoc.assign(
            src="gestionnaire", type_veh="tv/pl", obs=None
        ).drop_duplicates(["id_comptag_uniq", "periode"])
        dfAssocInsert["rang"] = (
            dfAssocInsert.assign(toto=1)
            .groupby("id_comptag_uniq")
            .toto.rank(method="first")
        )
        ...
        self.insert_bdd(
            ...
            tableComptage,
            dfAssocInsert[
                ["id_comptag_uniq", "periode", "src", "type_veh", "rang", "obs"]
            ].rename(columns={"id_comptag_uniq": "id_cptag_ref"}),
        )
        return dfAssoc

    def creerIndicsAssoc(
        self,
        dfAssoc,
        schema="comptage_assoc",
        tableAgreg="indic_agrege",
        tableHoraire="indic_horaire",
    ):
        """
        ...
        """
        dfIndicAgregeAssoc = dfAssoc.merge(
            self.recupererIdUniqComptageAssoc(dfAssoc.id_comptag_uniq.tolist()).rename(
                columns={"id_comptag_uniq": "id_comptag_uniq_assoc"}
            ),
            left_on="id_comptag_uniq",
            right_on="id_cptag_ref",
        )[["id_comptag_uniq_assoc", "indicateur", "valeur", "fichier"]]
        dfHoraireassoc = (
            self.dfHoraire.loc[
                self.dfHoraire.fichier.isin(dfIndicAgregeAssoc.fichier.tolist())
            ]
            .merge(
                dfIndicAgregeAssoc[["id_comptag_uniq_assoc", "fichier"]], on="fichier"
            )
            .drop(
                [
                    "id_comptag",
            ...
            .rename(columns={"id_comptag_uniq_assoc": "id_comptag_uniq"})
        )
        dfHoraireassoc["indicateur"] = dfHoraireassoc.indicateur.str.upper()
        self.insert_bdd(
            schema,
            tableAgreg,
            dfIndicAgregeAssoc.rename(
                columns={"id_comptag_uniq_assoc": "id_comptag_uniq"}
            ),
        )
...
class Comptag_Angouleme(Comptage):
            ...
            for f in files:
                ...
                if f.endswith(".mdb"):
                    try:
                        ...
                        listHshdr.append(
                            cpt.dicoTables["hshdr"].assign(fichier=fichier)
                        )
                        listDfDataBrutes.append(
                            cpt.formaterDonneesHoraires(
                                cpt.formaterDonneesIndiv(cpt.dfAgreg2Sens)
                            ).assign(fichier=fichier)
                        )
                    except ValueError:
    ...
    def assignerDossierRefEtAdresse(self, dFDataBrutes, dfHshdr):
        ...
        dFDataBrutes["nb_niv_dir"] = dFDataBrutes.spliPath.apply(
            lambda x: len(PureWindowsPath(x[1]).parents)
        )
        dfHshdr["nb_niv_dir"] = dfHshdr.spliPath.apply(
            lambda x: len(PureWindowsPath(x[1]).parents)
        )
        ...
        dFDataBrutes.loc[dFDataBrutes.nb_niv_dir == 4, "dir_ref"] = dFDataBrutes.loc[
            dFDataBrutes.nb_niv_dir == 4
        ].spliPath.apply(lambda x: PureWindowsPath(x[1]).parents[1].name)
        dFDataBrutes.loc[
            dFDataBrutes.nb_niv_dir.isin((2, 3)), "dir_ref"
        ] = dFDataBrutes.loc[dFDataBrutes.nb_niv_dir.isin((2, 3))].spliPath.apply(
            lambda x: PureWindowsPath(x[1]).parents[0].name
            if not "vers " in PureWindowsPath(x[1]).parts[-2]
            else PureWindowsPath(x[1]).parents[1].name
        )
        dfHshdr.loc[dfHshdr.nb_niv_dir == 4, "dir_ref"] = dfHshdr.loc[
            dfHshdr.nb_niv_dir == 4
        ].spliPath.apply(lambda x: PureWindowsPath(x[1]).parents[1].name)
        dfHshdr.loc[dfHshdr.nb_niv_dir.isin((2, 3)), "dir_ref"] = dfHshdr.loc[
            dfHshdr.nb_niv_dir.isin((2, 3))
        ].spliPath.apply(
            lambda x: PureWindowsPath(x[1]).parents[0].name
            if not "vers " in PureWindowsPath(x[1]).parts[-2]
            else PureWindowsPath(x[1]).parents[1].name
        ...
        dfHshdr["adresse"] = dfHshdr.apply(
            lambda x: re.sub(
                "(numero|n°)",
                "",
                re.search(
                    "(numero *[0-9]+|n° *[0-9]+)",
                    re.sub("é|è|ê", "e", x.Location.lower()),
                )[0],
            )
            + f" {x.Street}"
            if re.search("(numero|n°)", re.sub("é|è|ê", "e", x.Location.lower()))
            else np.NaN,
            axis=1,
        )
    ...
    def adresseUniques(self, dfHshdr):
        ...
        dfAdresse = (
            dfHshdr[["fichier", "dir_ref", "adresse"]]
            .groupby("dir_ref")
            .agg({"adresse": lambda x: uniformiserAdresse(x)})
            .reset_index()
        )
        return dfAdresse
    ...
    def assignerAdressesManuelles(self, dfAdresse, dicoAdresseMano):
        ...
        """
        # vrerif
        if not len(dicoAdresseMano) == len(
            dfAdresse.loc[dfAdresse["adresse"].isna()].dir_ref.unique()
        ):
            raise ValueError(
                "le nombre d'adresse inconnue et d'adresse fournie par dico doit etre le mm"
            )
        # assignation des nouvelles adresses
        dfAdresse2 = dfAdresse.merge(
            pd.DataFrame.from_dict(dicoAdresseMano, orient="index", columns=["adresse"])
            .reset_index()
            .rename(columns={"index": "dir_ref"}),
            on="dir_ref",
            how="left",
        )
        # verif finale
        dfAdresse2["adresse"] = dfAdresse2.apply(
            lambda x: x.adresse_x if not pd.isnull(x.adresse_x) else x.adresse_y, axis=1
        )
    ...
    def exporterCsvAGeocoder(self, dfAdresse2):
        ...
        """
        dfAdresse2.loc[
            dfAdresse2.adresse.apply(
                lambda x: False if re.search("[0-9]{2}\.[0-9]{4,9}", x) else True
            )
        ].assign(codepostal="16000", commune="Angoulême").to_csv(
            os.path.join(self.dossierSource, f"Comptage{self.annee}.csv")
        )
    ...
    def creerDfGeoloc(self, dfAdresse2):
        ...
        """
        gdfGeoloc = gp.read_file(
            os.path.join(self.dossierSource, f"Comptage{self.annee}_geoloc.csv"),
            encoding="utf-8",
        )  # C:\Users\martin.schoreisz\Documents\temp\OTV\Anglet\Comptage2019_geoloc.csv
        gdfGeoloc.geometry = gp.points_from_xy(
            x=gdfGeoloc["longitude"], y=gdfGeoloc["latitude"], crs=4326
        )
        gdfPoints = gp.GeoDataFrame(
            dfAdresse2.loc[
                dfAdresse2.adresse.apply(
                    lambda x: True if re.search("[0-9]{2}\.[0-9]{4,9}", x) else False
                )
            ],
            geometry=gp.points_from_xy(
                dfAdresse2.loc[
                    dfAdresse2.adresse.apply(
                        lambda x: True
                        if re.search("[0-9]{2}\.[0-9]{4,9}", x)
                        else False
                    )
                ].adresse.apply(lambda x: float(x.split(", ")[1])),
                dfAdresse2.loc[
                    dfAdresse2.adresse.apply(
                        lambda x: True
                        if re.search("[0-9]{2}\.[0-9]{4,9}", x)
                ...
                ].adresse.apply(lambda x: float(x.split(", ")[0])),
                crs="epsg:4326",
            ),
        )
        ...
        gdfGeoloc["id_comptag"] = gdfGeoloc.apply(
            lambda x: "Angouleme-"
            + re.sub("_{2,50}", "_", re.sub("(,|\.|'| +)", "_", x["adresse"].strip()))
            + f"-{round(float(x['longitude']), 4)};{round(float(x['latitude']),4)}"
            if not pd.isnull(x.codepostal)
            else "Angouleme-"
            + f"-{round(float(x['longitude']), 4)};{round(float(x['latitude']),4)}",
            axis=1,
        )
        gdfGeoloc["route"] = gdfGeoloc.apply(
            lambda x: re.sub("^([1-9]{1,3},)+", "", x.adresse)
            if not pd.isnull(x.codepostal)
            else np.NaN,
            axis=1,
        )
        ...
        gdfGeoloc["concession"] = False
        gdfGeoloc["type_poste"] = "ponctuel"
        gdfGeoloc["src_geo"] = gdfGeoloc.apply(
            lambda x: "adresse" if not pd.isnull(x.codepostal) else "manuelle", axis=1
        )
        gdfGeoloc["obs_geo"] = gdfGeoloc.apply(
            lambda x: f"geocodé avec 'mon géocodeur' de l'IGN ; qualite : {x.qualite} ; precision geocodage : {x['precision geocodage']} ; ID adresse : {x['ID adresse']}; adresse geocodee : {x['adresse geocodee']}"
            if not pd.isnull(x.codepostal)
            else "pas d'adressee à proximite",
            axis=1,
        )
        gdfGeoloc["x_l93"] = round(gdfGeoloc.geometry.x, 3)
        gdfGeoloc["y_l93"] = round(gdfGeoloc.geometry.y, 3)
    ...
    def creerIndicsTousFichiers(self, dFDataBrutes):
        ...
        dfDataBrutesIdcomptag = dFDataBrutes.reset_index().merge(
            self.gdfGeoloc[["dir_ref", "id_comptag"]], how="left"
        )
        # verif
        if dfDataBrutesIdcomptag.id_comptag.isna().any():
            raise ValueError("il manque un id_comptag sur une valeur")
        O.checkAttributsinDf(
            dfDataBrutesIdcomptag,
            attributsHoraire + ["jour", "id_comptag", "fichier", "indicateur"],
        )
        dfDataBrutesIdcomptag[
            "indicateur"
        ] = dfDataBrutesIdcomptag.indicateur.str.upper()
        self.dfHoraire = dfDataBrutesIdcomptag[
            attributsHoraire + ["jour", "id_comptag", "fichier", "indicateur"]
        ]
    ...
    def ajouterFichierConcatener(self):
        ...
        """
        dfFichiersConcat = self.dfHoraire.groupby("id_comptag").fichier.apply(
            lambda x: ", ".join([os.path.basename(e) for e in set(x)])
        )
        self.dfHoraire = self.dfHoraire.drop("fichier", axis=1, errors="ignore").merge(
            dfFichiersConcat.reset_index(), on="id_comptag"
        )
        self.dfIndicAgrege = self.dfIndicAgrege.drop(
            "fichier", axis=1, errors="ignore"
        ).merge(dfFichiersConcat.reset_index(), on="id_comptag")
        self.dfIndicAgrege["sens_cpt"] = self.dfIndicAgrege.fichier.apply(
            lambda x: "double sens" if len(x.split(",")) > 1 else "sens unique"
        )
    ...
    def transfererAssocSupp(self, dfPPVCptRef, dfPPVCptAssoc, listCptAssoc):
        ...
        """
        dfPPVCptAssoc = pd.concat(
            [dfPPVCptAssoc, dfPPVCptRef.loc[dfPPVCptRef.id_comptag.isin(listCptAssoc)]]
        )
        dfPPVCptRef = dfPPVCptRef.drop(
            dfPPVCptRef.loc[dfPPVCptRef.id_comptag.isin(listCptAssoc)].index
        )
        return dfPPVCptRef, dfPPVCptAssoc

    def insererCompteur(
        self, dfPPVSsDbl, CptAForcer=None, schema="comptage", table="compteur"
    ):
        """
        Preparer les donnes liees au compteur et les inserer (si besoin) dans la bdd
        in :
        ...
        """
        # creer les compteurs a inserer dans la bdd
        dfCptAcreer = dfPPVSsDbl.loc[
            (dfPPVSsDbl.id_comptag_bdd.isna())
            | (dfPPVSsDbl.id_comptag.isin(CptAForcer))
        ]
        if not dfCptAcreer.empty:
            compteur = (
                dfCptAcreer[[e for e in dfCptAcreer.columns if e in attBddCompteur]]
                .drop_duplicates()
                .copy()
            )
            # inserer
    ...
    def insererComptage(
        self, dfPPVSsDbl, CptAForcer=None, schema="comptage", table="comptage"
    ):
        """
        Preparer les donnes liees au comptages et les inserer dans la bdd
        in :
        ...
        dfPPVSsDblTemp["id_comptag_final"] = dfPPVSsDblTemp.apply(
            lambda x: x.id_comptag_bdd
            if not pd.isnull(x.id_comptag_bdd) and x.id_comptag not in CptAForcer
            else x.id_comptag,
            axis=1,
        )
        comptage = (
...
class Comptage_Dira(Comptage):
    ...
    def __init__(
        self,
        fichierTmjaParSection,
        dossierAnneeComplete,
        fichierComptageCarto,
        annee,
        table,
    ):
    ...
    def ouvrirMiseEnFormeFichierTmja(self):
        ...
        dfBase = pd.read_excel(
            self.fichierTmjaParSection,
            engine="odf",
            sheet_name=f"TMJM_{self.annee}",
            na_values=["No data"],
        )
        dfLimitee = dfBase.iloc[:, 0:14]
        mois, colonnes = [
            m
            for m in dfLimitee.iloc[0, 1:].tolist()
            if m in [a for v in dico_mois.values() for a in v]
        ], ["nom"] + dfLimitee.iloc[0, 1:].tolist()
        dfLimitee.columns = colonnes
        dfFiltree = (
            dfLimitee.loc[
                dfLimitee.apply(
                    lambda x: not all([pd.isnull(x[m]) for m in mois]), axis=1
                )
            ]
            .iloc[2:, :]
            .copy()
        ...
        dfFiltree.nom = dfFiltree.nom.apply(
            lambda x: re.sub("(é|è|ê)", "e", re.sub("( |_|-)", "", x.lower())).replace(
                "ç", "c"
            )
        )
        dfFiltree.rename(
            columns={
                c: k
                for k, v in dico_mois.items()
                for c in dfFiltree.columns
                if c not in ("nom", "MJA", "donnees_type")
                if c in v
            },
            inplace=True,
        )
        dfFiltree["nom_simple"] = dfFiltree.nom.apply(
            lambda x: x.split("mb")[0] + ")" if "mb" in x else x
        )
    ...
    def correspIdsExistant(self, table_cpt):
        ...
        """
        self.existant = compteur_existant_bdd(
            table_cpt, schema="comptage", dep=False, type_poste=False, gest="DIRA"
        )
        return pd.concat(
            [
                self.decomposeObsSupl(b, c, d)
                for b, c, d in zip(
                    self.existant.obs_supl.tolist(),
                    self.existant.id_comptag.tolist(),
                    self.existant.id_cpt.tolist(),
                )
            ],
            axis=0,
            sort=False,
    ...
    def jointureExistantFichierTmja(self, nomAttrFichierDira="nom_simple"):
        ...
        dfMens = dfFiltree.merge(
            self.dfCorrespExistant, left_on=nomAttrFichierDira, right_on="id_dira"
        )
        return dfMens.drop_duplicates(
            [k for k in dico_mois.keys()] + ["nom"]
        )  # car pb de doublons pas clair
    ...
    def verifValiditeMensuelle(self, dfMens):
        ...
        """
        verifValidite = dfMens.merge(
            dfMens.groupby("id_comptag")
            .id_dira.count()
            .reset_index()
            .rename(columns={"id_dira": "nb_lgn"}),
            on="id_comptag",
        )
        for idComptag in verifValidite.id_comptag.tolist():
            ...
            if (test.nb_lgn == 4).all():
                ...
                colInvalidesTmja = testTmja.columns.to_numpy()[
                    testTmja.isnull().any().to_numpy()
                ]
                verifValidite.loc[
                    (verifValidite["donnees_type"] == "tmja")
                    & (verifValidite["id_comptag"] == idComptag),
                    colInvalidesTmja,
                ] = -99
                # pour le pc_pl
                ...
                verifValidite.loc[
                    (verifValidite["donnees_type"] == "pc_pl")
                    & (verifValidite["id_comptag"] == idComptag),
                    colInvalidesTmja,
                ] = -99
                ...
                verifValidite.loc[
                    verifValidite["id_comptag"] == idComptag
                ] = verifValidite.loc[verifValidite["id_comptag"] == idComptag].fillna(
                    -99
                )
            else:
    ...
    def MiseEnFormeMensuelleAnnuelle(self, verifValidite):
        ...
        dfMensGrpTmja = (
            verifValidite.loc[verifValidite["donnees_type"] == "tmja"][
                ["id_comptag", "MJA"] + [k for k in dico_mois.keys()]
            ]
            .groupby("id_comptag")
            .sum()
            .assign(donnees_type="tmja")
        ...
        dfMensGrpPcpl = (
            pd.concat(
                [
                    verifValidite.loc[verifValidite["donnees_type"] == "pc_pl"][
                        [k for k in dico_mois.keys()] + ["MJA"]
                    ].astype("float64")
                    * 100,
                    verifValidite.loc[verifValidite["donnees_type"] == "pc_pl"][
                        ["id_comptag"]
        ...
        dfMensGrpPcpl = dfMensGrpPcpl.loc[
            dfMensGrpPcpl["MJA"] <= 99
        ]  # cas bizarre avec 100% PL
        dfMensGrp = (
            pd.concat([dfMensGrpPcpl, dfMensGrpTmja], axis=0, sort=False)
            .sort_values("id_comptag")
            .reset_index()
        )
        cond = dfMensGrp[[k for k in dico_mois.keys()]] > 0
        valMoins99 = dfMensGrp[[k for k in dico_mois.keys()]].where(cond, -99)
        dfMensGrpHomogene = pd.concat(
            [valMoins99, dfMensGrp[["id_comptag", "MJA", "donnees_type"]]],
            axis=1,
            sort=False,
    ...
    def MiseEnFormeAnnuelle(self, dfMensGrp, idComptagAexclure=None):
        ...
        idComptagAexclure = (
            idComptagAexclure
            if isinstance(idComptagAexclure, list)
            else [
                idComptagAexclure,
            ]
        )
        ...
        self.df_attr = (
            dfMensGrpFiltre.loc[dfMensGrpFiltre["donnees_type"] == "tmja"][["MJA"]]
            .rename(columns={"MJA": "tmja"})
            .merge(
                dfMensGrpFiltre.loc[dfMensGrpFiltre["donnees_type"] == "pc_pl"][
                    ["MJA"]
                ].rename(columns={"MJA": "pc_pl"}),
                how="left",
                right_index=True,
                left_index=True,
            )
    ...
    def MiseEnFormeMensuelle(self, dfMensGrp, idComptagAexclure=None):
        ...
        self.df_attr_mens = self.df_attr_mens.loc[
            ~self.df_attr_mens.index.isin(idComptagAexclure)
        ].copy()
        self.df_attr_mens.loc[
            self.df_attr_mens["donnees_type"] == "pc_pl", "janv":"dece"
        ] = self.df_attr_mens.loc[
            self.df_attr_mens["donnees_type"] == "pc_pl", "janv":"dece"
        ].applymap(
            lambda x: round(x, 2)
    ...
    def enteteFeuilHoraire(self, fichier, feuille):
        ...
        idDira = (
            re.sub(
                "ç", "c", re.sub("(é|è|ê)", "e", re.sub("( |_|-)", "", site.lower()))
            )
            + "("
            + re.sub(
    ...
    def miseEnFormeFeuille(self, fichier, feuille, nbHeure0Max=8, FlagHorsOTV=False):
        ...
        if not FlagHorsOTV:
            if (
                not idDira in self.dfCorrespExistant.id_dira.tolist()
            ):  # il n'y a pas de correspondance avec un point de comptage
                if (
                    not voie
                    in self.dfCorrespExistant.id_cpt.str.replace("_", " ").tolist()
                ):
                    raise self.BoucleNonConnueError(idDira)
                else:
                    flagVoie = True
        colonnes = ["jour", "type_veh"] + [
            "h" + c[:-1].replace("-", "_")
            for c in fichier[feuille].iloc[4, :].values
            if c[-1] == "h"
        ]
        df_horaire = fichier[feuille].iloc[
            5 : fichier[feuille]
            .loc[fichier[feuille].iloc[:, 0] == "Moyenne Jours"]
            .index.values[0]
            - 1,
            :26,
        ]
        ...
        if not FlagHorsOTV:
            if flagVoie:
                ...
                df_horaire = df_horaire.merge(
                    self.dfCorrespExistant.assign(
                        id_cpt=self.dfCorrespExistant.id_cpt.str.replace("_", " ")
                    ),
                    left_on="id_dira",
                    right_on="id_cpt",
                )
        ...
        if any(
            [
                (
                    len(
                        df_horaire.loc[
                            (df_horaire.isna().any(axis=1))
                            & (df_horaire["type_veh"] == t)
                        ]
                    )
                    > len(df_horaire.loc[df_horaire["type_veh"] == t]) / 2
                )
                or len(
                    df_horaire.loc[df_horaire["type_veh"] == t].loc[
                        df_horaire.loc[df_horaire["type_veh"] == t][
                            df_horaire.loc[df_horaire["type_veh"] == t] == 0
                        ].count(axis=1)
                        > nbHeure0Max
                    ]
                )
    ...
    def verif2SensDispo(self, df):
        ...
        dfJourValeur = (
            df.loc[
                df.voie.apply(
                    lambda x: re.sub(
                        "ç", "c", re.sub("(é|è|ê)", "e", re.sub("( |_)", "", x.lower()))
                    )
                ).isin(denominationSens)
            ]
            .groupby(["jour", "id_comptag"])
            .agg({"voie": lambda x: x.count(), "type_veh": lambda x: tuple(x)})
        )
        # liste des jours avec moins de 4 valeurs (i.e pas 2 sens en VL et PL) et un type veh en (VL,PL)
        JoursPb = dfJourValeur.loc[
            (dfJourValeur["voie"] == 2)
            & (dfJourValeur.type_veh.isin([("VL", "PL"), ("PL", "VL")]))
        ].index.tolist()
        dfFinale = df.loc[
            df.apply(lambda x: (x["jour"], x["id_comptag"]) not in JoursPb, axis=1)
        ].copy()
        return dfFinale
    ...
    def getIdEquiv(self, idDira):
        ...
        """
        try:
            idDira2 = self.dfCorrespExistant.loc[
                self.dfCorrespExistant["id_comptag"].isin(
                    self.dfCorrespExistant.loc[
                        self.dfCorrespExistant["id_dira"] == idDira
                    ].id_comptag.tolist()
                )
                & (self.dfCorrespExistant["id_dira"] != idDira)
            ].id_dira.values[0]
    ...
    def miseEnFormeFichier(
        self,
        nomFichier,
        nbJoursValideMin=7,
        dicoModifVerifValiditeHoraire=None,
        FlagHorsOTV=False,
        nbHeure0Max=8,
    ):
        """
        ...
        in :
            ...
            FlagHorsOTV : drapeau pour outre-passer les verifs liées aux références OTV
        """
        fichier = pd.read_excel(
            os.path.join(self.dossierAnneeComplete, nomFichier), sheet_name=None
        )  # A63_Ech24_Trimestre2_2019.xls
        dicoFeuille = {}
        listFail = (
            []
        )  # pour la gestion du cas où une des 2 feuilles de la section courantes est invalide, il faut pouvoir identifier l'autre et la virer
        for feuille in [k for k in fichier.keys() if k[:2] != "xx"]:
            ...
            if not FlagHorsOTV:
                try:
                    print(feuille)
                    if (
                        not dicoModifVerifValiditeHoraire
                        or nomFichier not in dicoModifVerifValiditeHoraire.keys()
                    ):
                        df_horaire, idDira = self.miseEnFormeFeuille(
                            fichier, feuille, nbHeure0Max=nbHeure0Max
                        )
                    else:
                        df_horaire, idDira = self.miseEnFormeFeuille(
                            fichier,
                            feuille,
                            dicoModifVerifValiditeHoraire[nomFichier],
        ...
        if (
            ...
            dfHoraireFichierFiltre = verifValiditeFichier(
                dfHoraireFichier, NbHeures0Max=nbHeure0Max
            )[
                0
            ]  # tri des feuilles sur le nb de valeusr NaN ou 0
        else:
            dfHoraireFichierFiltre = verifValiditeFichier(
                dfHoraireFichier,
                dicoModifVerifValiditeHoraire[nomFichier],
                NbHeures0Max=nbHeure0Max,
            )[0]
        if not FlagHorsOTV:
            # test si fichier comprenant des id_comptag à 2 sens
            dfFiltre = dfHoraireFichierFiltre.loc[
                dfHoraireFichierFiltre.id_comptag.isin(
                    self.dfCorrespExistant.set_index("id_comptag")
                    .loc[self.dfCorrespExistant.id_comptag.value_counts() == 2]
                    .index.unique()
                )
            ]
            if not dfFiltre.empty:
                ...
                try:
                    comparer2Sens(
                        dfHoraireFichierFiltre,
                        attributSens="voie",
                        attributIndicateur="type_veh",
                    )
                except SensAssymetriqueError as e:
                    dfHoraireFichierFiltre = dfHoraireFichierFiltre.loc[
                        dfHoraireFichierFiltre.apply(
                            lambda x: (x["jour"], x["id_comptag"])
                            not in zip(
                                e.dfCompInvalid.jour.tolist(),
                                e.dfCompInvalid.id_comptag.tolist(),
                            ),
                            axis=1,
                        )
                    ].copy()
                    ...
                    warnings.warn(
                        "attention, un des attribut nécéssaire à la vérification des sens est manquant"
                    )

        dfHoraireFichierFiltre = verifNbJoursValidDispo(
            dfHoraireFichierFiltre, nbJoursValideMin
        )[
            0
        ]  # tri sur id_comptag avec moins de 15 jours de donnees
    ...
    def concatTousFichierHoraire(self, dicoModifVerifValiditeHoraire=None):
        ...
        for i, fichier in enumerate(os.listdir(self.dossierAnneeComplete)):
            if fichier.endswith(".xls"):
                try:
                    listDf.append(
                        concatIndicateurFichierHoraire(
                            self.miseEnFormeFichier(
                                fichier,
                                dicoModifVerifValiditeHoraire=dicoModifVerifValiditeHoraire,
                            )
                        )
                    )
        ...
        dblSupprime = dfTousFichier.loc[
            dfTousFichier.duplicated(["id_comptag", "jour", "type_veh"], keep=False)
        ]
        dfTousFichier.drop_duplicates(["id_comptag", "jour", "type_veh"], inplace=True)
        idCptNonAffectes = self.dfCorrespExistant.loc[
            ~self.dfCorrespExistant["id_comptag"].isin(
                dfTousFichier.id_comptag.tolist()
            )
        ].id_comptag.tolist()
    ...
    def cptCartoVerif(self, tableurCarto):
        ...
        """
        if not (
            tableurCarto.loc[tableurCarto[f"carto_{self.annee}"] == "t"]
            .reset_index(drop="True")
            .equals(tableurCarto.loc[~tableurCarto.src.isna()].reset_index(drop="True"))
            and tableurCarto.loc[tableurCarto.diffusable == "t"]
            .reset_index(drop="True")
            .equals(tableurCarto.loc[~tableurCarto.src.isna()].reset_index(drop="True"))
        ):
            raise ValueError(
                f"les valeurs dans les champs carto_{self.annee}, src, diffusable ne sont pas concordante"
            )
        return tableurCarto
    ...
    def cptCartoForme(self, tableurCarto):
        ...
        if (
            donneesCarto.loc[donneesCarto.tmja.isna()]
            .obs.apply(
                lambda x: "Donnees Non Disponibles" not in x
                if not pd.isnull(x)
                else True
            )
            .any()
            or donneesCarto.loc[~donneesCarto.tmja.isna()]
            .obs.apply(
                lambda x: "Donnees Non Disponibles" in x if not pd.isnull(x) else False
            )
            .any()
        ):
            raise ValueError(
                f"les lignes sans valeur de TMJA doivent contenir la mention 'Donnees Non Disponibles' (et inversement) dans le champs obs "
            )

        donneesAgregees = (
            tableurCarto.loc[tableurCarto.carto_2020 == "t"]
            .groupby("id_comptag")
            .agg(
                {
                    "tmja": "sum",
                    "pc_pl": "mean",
                    "src": lambda x: tuple(x)[0],
                    "obs": lambda x: tuple(x)[0]
                    if not any(
                        [
                            "Donnees Non Disponibles" in y
                            if not pd.isnull(y)
                            else False
                            for y in tuple(x)
                        ]
                    )
                    else "Donnees Non Disponibles",
                }
            )
            .reset_index()
        )
    ...
    def cptCartoInsertUpdate(self):
        ...
        donneesAgregeesUpdate = donneesAgregees.merge(
            self.recupererIdUniqComptage(
                donneesAgregees.id_comptag.tolist(), self.annee
            ),
            on=("id_comptag"),
        )
        donneesAgregeesInsert = donneesAgregees.loc[
            ~donneesAgregees.id_comptag.isin(
                self.recupererIdUniqComptage(
                    donneesAgregees.id_comptag.tolist(), self.annee
                ).id_comptag.tolist()
            )
    ...
    def update_bdd_Dira(self, schema, table, nullOnly=True):
        ...
        """
        valeurs_txt = self.creer_valeur_txt_update(
            self.df_attr, ["id_comptag", "tmja", "pc_pl", "src", "fichier"]
        )
        dico_attr_modif = {
            f"tmja_{self.annee}": "tmja",
            f"pc_pl_{self.annee}": "pc_pl",
        ...
        if nullOnly:
            self.update_bdd(
                schema, table, valeurs_txt, dico_attr_modif, "c.tmja_2019 is null"
            )
        else:
            self.update_bdd(schema, table, valeurs_txt, dico_attr_modif)
    ...
    class BoucleNonConnueError(Exception):
        ...
        def __init__(self, idDira):
            Exception.__init__(
                self,
                f"la boucle {idDira} n'est pas dans le champs obs_supl d'un des points de la bdd",
            )
    ...
    class FeuilleInvalideError(Exception):
        ...
        def __init__(self, feuille, idDira):
            self.idDira = idDira
            Exception.__init__(
                self, f"la feuille {feuille} contient trop de valuers NaN ou 0"
            )
    ...
    class AucuneBoucleConnueError(Exception):
        ...
        def __init__(self, nomFichier):
            Exception.__init__(
                self,
                f"le fichier {nomFichier} ne comporte aucune boucle dans le champs obs_supl d'un des points de la bdd, ou toute les feuilles sont corrompues",
            )
...
class Comptage_Dirco(Comptage):
    ...
    def miseEnFormeMJA(self):
        ...
        """
        # import des données
        dfTmjaBrute = pd.read_excel(
            self.fichierTmja, skiprows=3, na_values=["ND", "#VALEUR !"]
        )
        # Mise en forme
        O.checkAttributsinDf(
            dfTmjaBrute,
        ...
        dfTraficBrut = (
            dfTmjaBrute.loc[
                (~dfTmjaBrute["trafic cumulé"].isna())
                & (dfTmjaBrute["trafic cumulé"] != "TV")
            ]
            .copy()
            .rename(columns={"trafic cumulé": "tmja", " PL": "pc_pl"})
        )
        dfTraficBrut["id_comptag"] = dfTraficBrut.apply(
            lambda x: f"{str(int(x['Dépt.']))}-{x['Route'].replace('RN', 'N')}-{str(int(x['PR']))}+{str(int(x['Abc']))}",
            axis=1,
        )
        dfTraficBrut["obs_supl"] = dfTraficBrut.apply(
            lambda x: f"site : {x['Nom de la station']}", axis=1
        )
        dfTraficBrut["fichier"] = os.path.basename(self.fichierTmja)
        dfTraficBrut["pc_pl"] = dfTraficBrut["pc_pl"] * 100
        dfTrafic = dfTraficBrut[
            ["id_comptag", "tmja", "pc_pl", "obs_supl", "fichier"]
        ].copy()
        return dfTrafic
    ...
    def miseEnFormeMJM(self, feuille):
        ...
        dfTmjmDirco["Route"] = dfTmjmDirco.Route.apply(
            lambda x: x if not isinstance(x, int) else f"RN{str(x)}"
        )
        dfTmjmDirco["pr"] = dfTmjmDirco.apply(
            lambda x: x["Route"].replace(" ", "") if "+" in x["Route"] else np.NaN,
            axis=1,
        ).fillna(method="bfill")
        dfTmjmDirco["voie"] = dfTmjmDirco.apply(
            lambda x: x["Route"].replace("RN", "N")
            if not "+" in x["Route"]
            else np.NaN,
            axis=1,
        ).fillna(method="ffill")
        dfTmjmDirco["dept"] = dfTmjmDirco.apply(
            lambda x: x["Intitulé du PM"] if not "+" in x["Route"] else np.NaN, axis=1
        ).fillna(method="ffill")
        dfTmjmDirco["id_comptag"] = dfTmjmDirco.apply(
            lambda x: f"{str(int(x['dept']))}-{x['voie']}-{str(x['pr'])}", axis=1
        )
        dfTmjmDirco["donnees_type"] = dfTmjmDirco["TMJA  "].apply(
            lambda x: "pc_pl" if x < 1 else "tmja"
        )
        dfTmjmDirco.drop(
            ["dept", "Intitulé du PM", "Route", "Unnamed: 1", "pr", "voie", "TMJA  "],
            axis=1,
            inplace=True,
        )
        dfTmjmDirco.rename(
            columns={
                c: k
                for k, v in dico_mois.items()
                for c in dfTmjmDirco.columns
                if c not in ("id_comptag", "donnees_type")
                if c in v
            },
            inplace=True,
        )
    ...
    def indicateurGlobalFeuilleMJM(self, dfTmjmDirco):
        ...
        """
        Agreg = dfTmjmDirco.merge(
            dfTmjmDirco.groupby("id_comptag").donnees_type.count(), on="id_comptag"
        ).rename(
            columns={"donnees_type_x": "donnees_type", "donnees_type_y": "nbLigne"}
        )
        dfTv2SensCompte = (
            Agreg.loc[(Agreg["donnees_type"] == "tmja") & (Agreg.nbLigne == 4)][
                [e for e in Agreg.columns if e not in ("donnees_type", "nbLigne")]
            ]
            .groupby(["id_comptag"])
            .sum()
            .assign(donnees_type="tmja")
        ...
        dfTv1SensCompte = (
            Agreg.loc[(Agreg["donnees_type"] == "tmja") & (Agreg.nbLigne == 2)][
                [e for e in Agreg.columns if e not in ("donnees_type", "nbLigne")]
            ]
            .groupby(["id_comptag"])
            .agg(lambda x: 2 * x)
            .assign(donnees_type="tmja", obs="1 seul sens *2")
        )
        dfPl = (
            Agreg.loc[Agreg["donnees_type"] == "pc_pl"]
            .fillna(-99)[
                [e for e in Agreg.columns if e not in ("donnees_type", "nbLigne")]
            ]
            .groupby(["id_comptag"])
            .mean()
            .applymap(lambda x: x * 100 if x > 0 else np.NaN)
            .assign(donnees_type="pc_pl")
        )
        return pd.concat(
            [dfTv2SensCompte, dfTv1SensCompte, dfPl], axis=0, sort=False
        ).sort_values(["id_comptag", "donnees_type"])
    ...
    def indicateurGlobalFichierMJM(self, cptARetirer):
        ...
        dfTouteFeuilles = (
            pd.concat(
                [
                    self.indicateurGlobalFeuilleMJM(
                        self.miseEnFormeMJM(self.dfSourceTmjm[k])
                    )
                    for k in self.dfSourceTmjm.keys()
                ],
                axis=0,
                sort=False,
            )
            .assign(annee=self.annee)
            .reset_index()
        )
        dfTouteFeuilles = corresp_nom_id_comptag(dfTouteFeuilles)
        cptARetirer = (
            cptARetirer
            if isinstance(cptARetirer, list)
            else [
                cptARetirer,
            ]
        )
        dfTouteFeuilles = dfTouteFeuilles.loc[
            ~dfTouteFeuilles.id_comptag.isin(cptARetirer)
        ]
        # mettre le TMJA en type integer
        dfTouteFeuilles.loc[
            dfTouteFeuilles.donnees_type == "tmja", [k for k in dico_mois.keys()]
        ] = dfTouteFeuilles.loc[
            dfTouteFeuilles.donnees_type == "tmja", [k for k in dico_mois.keys()]
        ].applymap(
    ...
    def miseEnFormeFichierTmjaPourHoraire(self):
        ...
        def nomIdFichier(route, debStation, finStation, nomStation):
            if route == "A20":
                ...
                dico_corresp = {
                    "AYRON": "Station MBV86.O",
                    "EPANOURS": "Station MBW87.J",
                    "FLEURE": "Station MBV86.J",
                    "GENIEC": "Station MBV86.H",
                    "LA CROIX CHAMBERT": "Station MBX79.B",
                    "LE VINCOU": "Station MBW87.K",
                    "LOUBLANDE": "Station MBX79.A",
                    "MAISONNEUVE": "Station MBW87.A",
                    "MOULINET": "Station MBV86.A",
                    "MOULISMES": "Station MBW87.I",
                    "MILETERIE CHU": "MBV86.F",
                    "MARGOUILLET": "MBV86.G",
                    "BUXEROLLES": "MBV86.W",
                    "LA FOLIE": "MBV86.X",
                    "MIGNEAUXANCES": "MBV86.S",
                    "CHARDONCHAMP": "MBV86.Y",
                    "MIGNALOUX": "MBV86.V",
                }
        ...
        dfFichierTmja = pd.read_excel(
            self.fichierTmja, skiprows=3, na_values=["ND", "#VALEUR !"]
        )
        # dfFichierTmja.columns=dfFichierTmja.iloc[2].values
        dfFichierTmja = (
            dfFichierTmja.loc[~dfFichierTmja["Nom de la station"].isna()]
            .iloc[1:]
            .copy()
        )
        dfFichierTmja.columns = [
            "route",
            "dept",
            "pr",
            "absc",
        ...
        dfFichierTmja["pr"] = dfFichierTmja.pr.apply(
            lambda x: int(x) if not pd.isnull(x) else -999
        )
        dfFichierTmja.absc = dfFichierTmja.absc.apply(
            lambda x: int(x) if not pd.isnull(x) else x
        )
        dfFichierTmja["idFichier"] = dfFichierTmja.apply(
            lambda x: nomIdFichier(
                x["route"], x["debStation"], x["finStation"], x["nomStation"]
            ),
            axis=1,
        )
        dfFichierTmja["id_comptag"] = dfFichierTmja.apply(
            lambda x: f"{int(x['dept'])}-{x['route'].replace('RN','N')}-{x['pr']}+{x['absc']}",
            axis=1,
        )
    ...
    def ouvrirFichierHoraire(self, fichier):
        ...
        try:
            ...
            raise ValueError(
                f"il n'y a pas de feuille avec 'xxcpt' dans le fichier {fichier}"
            )
    ...
    def jointureHoraireIdcpt(self, fichier, dfFichier, dfFichierTmja):
        ...
        if "A20" in fichier.upper():
            dfFichier["Station"] = dfFichier.Station.apply(
        ...
        return dfFichier.merge(
            dfFichierTmja.loc[~dfFichierTmja.idFichier.isna()][
                ["idFichier", "id_comptag"]
            ],
            left_on="Station",
            right_on="idFichier",
            how="left",
    ...
    def miseEnFormeHoraire(self, dfHoraireAgregee):
        ...
        dfDonnees["heure"] = dfDonnees.apply(
            lambda x: f"h{str(x['sequence'].hour)}_{str(x['sequence'].hour+1)}", axis=1
        )
        dfSc = dfDonnees.loc[dfDonnees["Code Canal"].isin((0, 1))].copy()
        dfSc["voie"] = dfSc["Code Canal"].apply(
            lambda x: "sens 1" if x == 0 else "sens 2"
        )  # a ameliorer si besoin prise en compte Bretelle et autre
    ...
    def correspondanceStationIdcpt(self, dfSc):
        ...
        return (
            dfSc.groupby("id_comptag")["Code Station"]
            .unique()
            .apply(lambda x: x[0])
            .reset_index()
            .rename(columns={"Code Station": "codestation"})
        )
    ...
    def horaireParSens(self, dfSc):
        ...
        dfScParSens = (
            dfSc[
                ["id_comptag", "jour", "heure", "mesure", "type_veh", "voie", "fichier"]
            ]
            .pivot(
                index=["id_comptag", "jour", "type_veh", "voie", "fichier"],
                columns="heure",
                values="mesure",
            )
            .reset_index()
        ...
        if not dfJourIdcptARetirer.empty:
            for idCpt in dfJourIdcptARetirer.id_comptag.unique():
                ...
                dicoCptError[idCpt] = {
                    "typeError": "jours supprimes",
                    "df": dfJourIdcptARetirer.loc[
                        dfJourIdcptARetirer.id_comptag == idCpt
                    ].copy(),
                }
        for idCpt in dfScParSensValide.id_comptag.unique():
            try:
                dfComp = comparer2Sens(
                    dfScParSensValide.loc[dfScParSensValide["id_comptag"] == idCpt],
                    attributIndicateur="type_veh",
                )[1]
                ...
                dicoCptError[idCpt] = {
                    "typeError": "sens assymetriques",
                    "df": e.dfComp,
                }
        dfScParSensOk = dfScParSensValide.loc[
            ~dfScParSensValide["id_comptag"].isin(dicoCptError.keys())
        ]
    ...
    def horaire2SensFormatBdd(self, dfScParSensOk):
        ...
        dfHoraireValidNbJourOk, idCptInvalid, dfCptInvalid = verifNbJoursValidDispo(
            dfSc2Sens, 10
        )
    ...
    def tousFichierHoraires(self, dfFichierTmja):
        ...
        for fichier in [
            os.path.join(root, f)
            for (root, dirs, files) in os.walk(self.dossierHoraire)
            for f in files
        ]:
            print(fichier)
            ...
            if typeFichier == "station unique":
                dfFichierHoraire = self.jointureHoraireIdcpt(
                    fichier, dfFichier, dfFichierTmja
                )
                dfFichierHoraire = self.miseEnFormeHoraire(dfFichierHoraire)
                dfScParSensValide, dicoCptok, dicoCptError = self.horaireParSens(
                    dfFichierHoraire
                )
                ...
                for k, v in dicoCptok.items():
                    dicoCptOkTot[k] = v
                dfFinale, idCptInvalid, dfCptInvalid = self.horaire2SensFormatBdd(
                    dfScParSensValide
                )
                ...
                for station in dfFichier.Station.unique():
                    ...
                    dfFichierStationUniq = dfFichier.loc[
                        dfFichier.Station == station
                    ].copy()
                    dfFichierStationUniq[
                        "Station"
                    ] = dfFichierStationUniq.Station.str.upper()
                    dfFichierHoraire = self.jointureHoraireIdcpt(
                        fichier, dfFichierStationUniq, dfFichierTmja
                    )
                    dfFichierHoraire = self.miseEnFormeHoraire(dfFichierHoraire)
                    try:
                        (
                            ...
                            dfFichierHoraire.drop_duplicates(
                                [
                                    "id_comptag",
                                    "jour",
                                    "type_veh",
                    ...
                    if idCptInvalid:
                        for e in idCptInvalid:
                            print("fichier inavlid duree donees : {e}")
                            dicoCptError[e] = dfCptInvalid.loc[
                                dfCptInvalid.id_comptag == e
                            ]
                    listDfFinale.append(
                        dfFinale.assign(fichier=os.path.basename(fichier))
                    )
...
class PasAssezMesureError(Exception):
    ...
    def __init__(self, nbjours):
        Exception.__init__(
            self,
            f"le fichier comporte moins de 7 jours de mesures. Nb_jours: : {nbjours} ",
        )
...
class ComptageMultipleSurTronconHomogeneError(Exception):
    ...
    def __init__(self, refTroncHomo):
        Exception.__init__(
            self,
            f"le troncon homogene {','.join(refTroncHomo)} supporte plus que 1 comptage de reference",
        )
...
class FormatError(Exception):
    ...
    def __init__(self, formatDonnees):
        Exception.__init__(
            self, f"le format {formatDonnees} n'est pas prevu dans le code"
        )
